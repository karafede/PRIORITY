...
import os
# os.chdir('D:/Federico/Mobility/flask_app/static')
...
from flask import Flask, render_template, request, render_template_string, make_response, abort, jsonify, redirect, session
...
def mob_privata():
    ...
    session["FCD_day"] = '2024-10-15'

    session["ZMU_day_start"] = '2024-10-10'
    session["ZMU_day_end"] = '2024-10-11'
    session['input_data_range'] = "10/12/2024 - 10/13/2024"
    session["id_stay_type"] = 21
    print("stay points ID;", session["id_stay_type"])
    session["indicator_type"] = 102
    session["province_number"] = 15
    session["lat_ORIGIN_province"] = 41.8946
    session["lon_ORIGIN_province"] = 12.4873
    
    
    #### ----- load last SAVED LATITUDE & LONGITUDE ---- ################################
    stored_lat_files = glob.glob(path_app + "static/params/selected_lat_*.txt")
    stored_lat_files.sort(key=os.path.getmtime)
    stored_lat_file = stored_lat_files[len(stored_lat_files) - 1]

    with open(stored_lat_file) as file:
        selected_lat = file.read()
    print("selected_lat-------I AM HERE-----------: ", selected_lat)
    session["lat_ORIGIN_province"] = selected_lat

    #### ----------------------------------------- ###############################
    stored_lon_files = glob.glob(path_app + "static/params/selected_lon_*.txt")
    stored_lon_files.sort(key=os.path.getmtime)
    stored_lon_file = stored_lon_files[len(stored_lon_files) - 1]

    with open(stored_lon_file) as file:
        selected_lon = file.read()
    print("selected_lon-------I AM HERE-----------: ", selected_lon)
    session["lon_ORIGIN_province"] = selected_lon
    ...
    #### ----- check for session ----> PROVINCE NUMBER
    stored_province_number_files = glob.glob(path_app + "static/params/selected_province_number_*.txt")
    stored_province_number_files.sort(key=os.path.getmtime)
    stored_province_number_file = stored_province_number_files[len(stored_province_number_files) - 1]

    with open(stored_province_number_file) as file:
        selected_province_number = file.read()
    print("selected_province_number-------I AM HERE-----------: ", selected_province_number)
    session["province_number"] = selected_province_number
...
def impacts_page():
    ...
    session["ZMU_day_start"] = '2022-10-10'
    ...
    """
    ##Cost of emissions
    stored_costs_type_files = glob.glob(path_app + "static/params/selected_costs_type_*.txt")
    stored_costs_type_files.sort(key=os.path.getmtime)
    stored_costs_type_file = stored_costs_type_files[len(stored_costs_type_files) - 1]
    with open(stored_costs_type_file) as file:
        selected_costs_type = file.read()
    print("selected_costs_type-------I AM HERE-----------: ", selected_costs_type)
    session["costs_type"] = selected_costs_type

    ## external costs
    stored_external_type_files = glob.glob(path_app + "static/params/selected_external_type_*.txt")
    stored_external_type_files.sort(key=os.path.getmtime)
    stored_external_type_file = stored_external_type_files[len(stored_external_type_files) - 1]
    with open(stored_external_type_file) as file:
        selected_external_type = file.read()
    print("selected_external_type-------I AM HERE-----------: ", selected_external_type)
    session["external_type"] = selected_external_type
    """
...
def modal():
    ...
    ### load modal percentage value ---- ################################################
    stored_modal_files = glob.glob(path_app + "static/params/selected_modal_pt_*.txt")
    stored_modal_files.sort(key=os.path.getmtime)
    stored_modal_file = stored_modal_files[len(stored_modal_files) - 1]
    with open(stored_modal_file) as file:
        selected_modal_pt = file.read()
    print("selected_modal_pt-------I AM HERE-----------: ", selected_modal_pt)
    session["modal_pt"] = selected_modal_pt

    ## 0.8; 0.85; 0.9; 0.95

    return render_template("index_zmu_modal_shift.html")
...
def GTFS_hour_selector():
    if request.method == "POST":
        ...
        # selected_GTFS_day = '2019-10-09'

        # global selected_GTFS_hour
        # selected_GTFS_hour = request.form["GTFS_hour"]
        ...
        gdf_stop_buses.to_file(filename= path_app +'static/gtfs_stops_by_routes_filtered.geojson',
                             driver='GeoJSON')
        ## add "var name" in front of the .geojson file, in order to properly loat it into the index.html file
        with open(path_app + 'static/gtfs_stops_by_routes_filtered.geojson', 'r+', encoding='utf8', errors='ignore') as f:
            old = f.read()  # read everything in the file
            f.seek(0)  # rewind
        ...
        gtfs_trips_by_routes.to_file(filename=path_app + 'static/gtfs_trips_by_routes_filtered.geojson',
                               driver='GeoJSON')
        ## add "var name" in front of the .geojson file, in order to properly loat it into the index.html file
        with open(path_app + "static/gtfs_trips_by_routes_filtered.geojson", "r+", encoding="utf8", errors='ignore') as f:
            old = f.read()  # read everything in the file
            f.seek(0)  # rewind
...
def choose_zmu_index_routing():
    ...
    # import os
    #if os.path.exists(path_app + "static/filtered_od_routing.geojson"):
    #    os.remove(path_app + "static/filtered_od_routing.geojson")
    #else:
...
def redirect_zmu_paths_routing():
    ...
    filtered_od_routing = json.load(filtered_od_routing)
        # with open(path_app + 'static/filtered_od_routing_' + session.sid + '.geojson', "r") as file:
        #    filtered_od_routing = json.load(file)


    # with open(path_app + 'static/filtered_od_routing.geojson', "r") as file:
    #    AAA = json.load(file)
...
def process():
    ...
    try:
        with open(path_app + "static/params/selected_province_number_" + session.sid + ".txt", "r") as file:
            selected_province_number = file.read()
        print("selected_province_number-------I AM HERE-----------: ", selected_province_number)
    except:
        stored_province_number_files = glob.glob(path_app + "static/params/selected_province_number_*.txt")
        ...
        with open(stored_province_number_file) as file:
            selected_province_number = file.read()
        print("stored_province_number-------I AM HERE-----------: ", selected_province_number)
    ...
    zmu_zones = text('''SELECT * from landuse.zones''')
...
def pois():
    ...
    if len(pois_gdf) > 0:
        ...
        # get ZMU containing the PARKINGS locations
        POIS_parking = gpd.sjoin(ZMU_ROMA, POIS_parking, how='right',
                                           predicate='intersects')
        POIS_parking = pd.DataFrame(POIS_parking)
        ##---> group by PARKINGS and ZMUs
        aggr_PARKINGS_ZMU = POIS_parking.groupby(['zmu'])['zmu'].agg('count').reset_index(name='parking_area_counts')
        aggr_PARKINGS_ZMU['zmu'] = aggr_PARKINGS_ZMU.zmu.astype('int')
        ## merge with ZMU_ROMA
        ZMU_ROMA_PARKINGS = pd.merge(aggr_PARKINGS_ZMU, ZMU_ROMA, on=['zmu'], how='left')
        ## save to .geojson file
        ZMU_ROMA_PARKINGS = gpd.GeoDataFrame(ZMU_ROMA_PARKINGS)
        ZMU_ROMA_PARKINGS.to_file(filename=path_app + 'static/zmu_PARKINGS_ROMA_2011.geojson',
                               driver='GeoJSON')
        ## add "var name" in front of the .geojson file, in order to properly loat it into the index.html file
        with open(path_app + "static/zmu_PARKINGS_ROMA_2011.geojson", "r+") as f:
            old = f.read()  # read everything in the file
            f.seek(0)  # rewind
            f.write("var zmu_PARKINGS_counts = \n" + old)  # assign the "var name" in the .geojson file
        """
    ...
    tags_residential = {'amenity': True,
                        'building': ['apartments', 'detached', 'hotel', 'house', 'residential', 'semidetached_house', 'commercial', 'dormitory'
                                     'industrial', 'dormitory', 'office', 'supermarket', 'warehouse', 'mosque', 'train_station', 'transportation',
                                     'sports_hall', 'stadium']}
...
def GTFS_selector():
    if request.method == "POST":
        ## declare a global variable
        # global selected_GTFS_data
        ## selected_GTFS_data= '20230405'
...
def TPL_selector():
    if request.method == "POST":
        ...
        ## declare a global variable
        # global selected_TPL_line
        # selected_TPL_line = request.form["line"]
...
def TPL_hour_selector():
    if request.method == "POST":
        ...
        #########-->>>>>>>>>>##########################################################
        # route_TPL = pd.read_csv("static/route_TPL.csv")
        # print("I read the route_TPL file")

        # print("selected_TPL_line:", str(selected_TPL_line))
        # print("selected_TPL_hour: ", int(selected_TPL_hour))

        ## add slected hour and selected TPL line to a list
        # global selected_TPL_params
        # selected_TPL_params = [selected_GTFS_data, selected_TPL_line, selected_TPL_hour]
        # print("selected_TPL_params:", selected_TPL_params)

        # route_TPL = route_TPL[route_TPL['route_ID'] == str(selected_TPL_line)]
        # route_TPL = route_TPL[route_TPL['hour'] == int(selected_TPL_hour)]
        # print(route_TPL)
        ...
        day = GTFS_dict[selected_GTFS_data]
        print("GTFS day:", day)
        stop_times = pd.read_csv(path + '/' +  day + '/' + 'stop_times.txt')   ## to get the name of the line --->route_id and name
        trips = pd.read_csv(path + '/' +  day + '/' + 'trips.txt')   ## to get "trip_id" and "shape_id" and "route_id" ("line")
        shapes = pd.read_csv(path + '/' +  day + '/' + 'shapes.txt')   ## to get "shape_id" and "lat", "lon" of each stop and "shape_pt_sequence"
        routes = pd.read_csv(path + '/' +  day + '/' + 'routes.txt')   ## "route_id" is the "line"
        ...
        timetable_animation = timetable[['trip_id', 'arrival_time', 'stop_id', 'stop_sequence', 'hour', 'stop_lat', 'stop_lon', 'shape_id', 'route_short_name', 'trip_headsign']]
        # route_id = '628'
        # hour = 12
...
def TPL_trip_selector():
    if request.method == "POST":
        ...
        # route_TPL = []
        # for idx_trip, trips_IDS in enumerate(all_trips_IDS):
        # print(trips_IDS)
        # slected_trip_ID = trip_dict[2]

        # selected_trip_ID = 10
        # selected_trip_ID = int(selected_trip_ID)
        # selected_trip_id = trip_dict[selected_trip_ID]
        ...
        if len(trip) > 0:
            ...
            for u in keys_shapes:
                shape_dict[u] = shape_path[['shape_pt_lat', 'shape_pt_lon']][shape_path['shape_pt_sequence'] == u].values.tolist()
            ...
            for v in keys_shape_start_stop:
                shape_start_stop[v] = start_end_stop[['stop_lat', 'stop_lon']][start_end_stop['stop_sequence'] == v].values.tolist()
            ...
            selected_shape_path = shape_path[(shape_path['shape_pt_sequence'] >= 1) & (shape_path['shape_pt_sequence'] <= end_stop_sequence_ID)]
            ##-->>> sort by sequence
            # trip_path = trip.sort_values(['stop_sequence'])
            # make a polyline:
            trip_poline_stop = [[x, y] for (x, y) in zip(selected_shape_path.shape_pt_lat, selected_shape_path.shape_pt_lon)]
            ## append last stop to list...
            # end_end_stop = end_stop[['stop_lat', 'stop_lon']].values.tolist()
            # trip_poline_stop.append(end_end_stop[0])
            ...
            try:
                ...
                selected_stops_trip =  pd.merge(stops_lat_lon, selected_trip, on=['stop_lat','stop_lon'], how='left')
                ## drop duplicated and keep the first
                selected_stops_trip = selected_stops_trip.drop_duplicates(subset=['stop_lat','stop_lon'], keep='first')
                selected_stops_trip = selected_stops_trip[['stop_lon', 'stop_lat', 'route_short_name', 'trip_headsign', 'stop_id']]
                ...
                all_stops_and_times = all_stops_and_times[
                    ['arrival_time', 'stop_lat', 'stop_lon', 'route_short_name', 'trip_headsign', 'stop_id', 'stop_sequence', 'trip_id']]
                ## merge with "stops" ti find the name of each stop
                all_stops_and_times['stop_id'] = all_stops_and_times['stop_id'].astype(str)
                all_stops_and_times = pd.merge(all_stops_and_times, stops[['stop_id', 'stop_name']], on=['stop_id'], how='left')
                df_stops_and_times = all_stops_and_times[['arrival_time', 'route_short_name', 'stop_name', 'trip_headsign', 'stop_sequence', 'trip_id']]
...
def stop_timelines():
    ...
    df_stops_and_times = df_stops_and_times[['arrival_time', 'route_short_name', 'stop_name','trip_headsign', 'stop_sequence', 'trip_id']]
...
def data_type_selector():
    ...
    if request.method == "POST":
        ...
        #### ----- check for session ----> data type (FCD, SYNTHETIC)
        stored_data_type_files = glob.glob(path_app + "static/params/selected_data_type_*.txt")
        stored_data_type_files.sort(key=os.path.getmtime)  ### ----> from oldest to newest
        list_stored_files = stored_data_type_files
        for idx, element in enumerate(list_stored_files):
            # print(element)
            # print(idx)
            if idx < len(list_stored_files) -3:
                print(idx)
                print(element)
                os.remove(element)
        stored_data_type_file = stored_data_type_files[len(stored_data_type_files) - 1]  ### ---> keep the newest

        '''
        import datetime
        ## find date of last file
        time_last_file_seconds = os.path.getctime(stored_data_type_file)
        ### ----- delete files oldest than the last date....
        import time
        time_now = time.time()
        list_timeseconds_files = [os.path.getctime(item) for item in stored_data_type_files]
        ### calculate difference (in number of days):
        ### if the oldest files are older than one day, then delete them....
        list_diff = [(time_now - item) / (24 * 3600) for item in list_timeseconds_files]
        for idx, elapsed_time in enumerate(list_diff):
            if elapsed_time > 1:
                print("---- OK --------", "idx:", idx)
                ## remove files in the "stored_data_type_files" list at the idx position
                print(stored_data_type_files[idx])
                os.remove(stored_data_type_files[idx])
        '''
...
@app.route('/province_selector/', methods=['GET', 'POST'])
def province_selector():
    import glob

    if request.method == "POST":

        session["province_number"] = request.form.get("province_number")
        selected_province_number = session["province_number"]
        print("selected_province_number:", selected_province_number)
        selected_province_number = str(selected_province_number)


        ## --------- SELECT DATA TYPE and redirect to new .html page -------- ##############
        selected_province_number = int(selected_province_number)
        if selected_province_number == 58:
            province = 'roma'
            print("roma")
            print("selected_province_number:--------selector", selected_province_number, province)


        if selected_province_number == 15:
            province = 'milano'
            print("milano")
            print("selected_province_number:--------selector", selected_province_number, province)


        elif selected_province_number == 48:
            province = 'firenze'
            print("firenze")
            print("selected_province_number:--------selector", selected_province_number, province)

        elif selected_province_number == 37:
            province = 'bologna'
            print("bologna")
            print("selected_province_number:--------selector", selected_province_number, province)

        elif selected_province_number == 10:
            province = 'genova'
            print("genova")
            print("selected_province_number:--------selector", selected_province_number, province)

        elif selected_province_number == 63:
            province = 'napoli'
            print("napoli")
            print("selected_province_number:--------selector", selected_province_number, province)

        elif selected_province_number == 82:
            province = 'palermo'
            print("palermo")
            print("selected_province_number:--------selector", selected_province_number, province)
        ...
        with open(path_app + "static/params/selected_province_number_" + session.sid + ".txt", "w") as file:
            file.write(str(selected_province_number))
        ...
        selected_province_number = session["province_number"]
        #### ---- find the centroid of the selected province ------
        #### load .geojason file of the provicnes in ITALY
        ...
        province_italia = gpd.read_file(path_app + "static/ProvCM01012025_g_WGS84.geojson")
        ##.... get CENTROIDS of each geometry
        province_italia[
            'centroid'] = province_italia.representative_point()  ## the "representative point" is VERY similar to the centroid.
        province_italia['centroid'] = province_italia.apply(lambda x: [y for y in x['centroid'].coords], axis=1)
        zoom_coords = province_italia[province_italia['COD_PROV'] == int(selected_province_number)]

        ## get longitude and latitude to CENTER the map over the centroid of the PROVINCE when select the province
        import shapely
        zoom_coords = shapely.get_coordinates(zoom_coords.centroid, include_z=True).tolist()

        session["lat_ORIGIN_province"] = zoom_coords[0][1]
        session["lon_ORIGIN_province"] = zoom_coords[0][0]
        
        ###### ---- save coordinates according to current session ---- ###########################
        
        with open(path_app + "static/params/selected_lat_" + session.sid + ".txt", "w") as file:
            file.write(str(session["lat_ORIGIN_province"]))

        with open(path_app + "static/params/selected_lon_" + session.sid + ".txt", "w") as file:
            file.write(str(session["lon_ORIGIN_province"]))
            
            
        ###### ------ clean old files from previous sessions ---- ##########################
        import glob
        stored_lat_files = glob.glob(path_app + "static/params/selected_lat_*.txt")
        stored_lat_files.sort(key=os.path.getmtime)  ### ----> from oldest to newest
        list_stored_files = stored_lat_files
        for idx, element in enumerate(list_stored_files):
            # print(element)
            # print(idx)
            if idx < len(list_stored_files) - 3:
        ...
        stored_lon_files = glob.glob(path_app + "static/params/selected_lon_*.txt")
        stored_lon_files.sort(key=os.path.getmtime)  ### ----> from oldest to newest
        list_stored_files = stored_lon_files
        for idx, element in enumerate(list_stored_files):
            # print(element)
            # print(idx)
            if idx < len(list_stored_files) - 3:
        ...
        print("---- I AM HERE ----> PROVINCE SELECTOR")
        print(session["lat_ORIGIN_province"])

        return render_template("index_zmu_select_day.html",
                               session_lat_ORIGIN_province=session["lat_ORIGIN_province"],
                               session_lon_ORIGIN_province=session["lon_ORIGIN_province"] )
...
def ZMU_hourrange_selector():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            selected_ZMU_day_start = '2024-10-04'
            session["ZMU_day_start"] = selected_ZMU_day_start
            print("using stored variable ZMU_day_start: ", session["ZMU_day_start"])
        ...
        try:
            ...
            selected_ZMU_day_end = '2024-10-15'
            session["ZMU_day_end"] = selected_ZMU_day_end
        ...
        with open(path_app + "static/params/selected_ZMU_hourrange_" + session.sid + ".txt", "w") as file:
            file.write(str(selected_ZMU_hourrange))

        print("selected_ZMU_hourrange-------I AM HERE-----------: ", selected_ZMU_hourrange)
        ...
        """
        hourrange_id = "M1"
        selected_province_number = '48'     ### FIRENZE
        selected_ZMU_day_start = '2024-10-03'
        selected_ZMU_day_end = '2024-10-04'
        selected_province_number = int(selected_province_number)
        stay_type = 'H'
        selected_stay_type = 21
        ...
        if selected_data_type == 11:  ## ------->>>>>>>  FCD data <<<<<-------------- ######
            ...
            if selected_province_number == 99:
                ...
                origin_destination_routes_day.rename({'tripdistance_m': 'dist'}, axis=1, inplace=True)
            ...
            # origin_destination_routes_day =  origin_destination_routes_day[origin_destination_routes_day.p_after > 1800]
            origin_destination_routes_day['speed'] = ((origin_destination_routes_day.dist)/1000) / ((origin_destination_routes_day.tt)/3600)
            ### consider Breaktims > 30 minutes
            ...
            ##### ------ redirect to the ZONING user's choice -----##############################################
            if selected_province_number == 58:
                print("--------------- province: Rome")
                #### ----- traffic zones in ROME
                ZMU_ROMA_with_population = gpd.read_file(path_app + "static/zmu_aggregated_POP_ROMA_2015_new.geojson")
            if selected_province_number == 48:
                print("--------------- province: Firenze")

                #### ---- CITY COUNCILS  IN ITALY ----- ################################################################
                ## get ZMU zones
                city_councils_zones = text('''SELECT * from landuse.zones''')

                geom_wgs84 = text('''SELECT
                           ST_Transform(geom, 4326)
                           FROM
                           landuse.zones''')

                with engine.connect() as conn:
                    res = conn.execute(city_councils_zones).all()
                ...
                with engine.connect() as conn:
                    res = conn.execute(geom_wgs84).all()

                geom_wgs84 = pd.DataFrame(res)
                geom_wgs84.rename({'st_transform': 'geom'}, axis=1, inplace=True)

                geom_wgs84['geom'] = geom_wgs84.apply(wkb_tranformation, axis=1)
                geom_wgs84 = gpd.GeoDataFrame(geom_wgs84)

                city_councils_zones = city_councils_zones[['area', 'zmu', 'comune', 'nome_comun', 'quartiere', 'pgtu',
                                                           'municipio', 'type', 'population', 'addetti', 'nscuole',
                                                           'natt', 'info',
                                                           'pop_mag15', 'pop_tot', 'occ', 'occ_a', 'occ_ba', 'stu_sup',
                                                           'stu_uni',
                                                           'altro', 'add_comm', 'add_spbl', 'add_sprv', 'add_scu',
                                                           'add_uni',
                                                           'add_altri', 'add_tot', 'id']]
                city_councils_zones['geometry'] = geom_wgs84['geom']
                ZMU_ROMA_with_population = gpd.GeoDataFrame(city_councils_zones)


            if selected_province_number == 99:

                print("--------------- province: italia_ev")
                #### ---- CITY COUNCILS  IN ITALY ----- ################################################################
                ## get ZMU zones
                city_councils_zones = text('''SELECT * from landuse.zones''')
                ...
                city_councils_zones = gpd.GeoDataFrame(city_councils_zones)
                city_councils_zones.rename({'geom': 'geometry'}, axis=1, inplace=True)

                ## reference system = 6875 (in meters)
                # ZMU_ROMA = GeoDataFrame.set_crs(ZMU_ROMA, crs="EPSG:6875", allow_override=True)
                city_councils_zones = city_councils_zones.set_geometry("geometry")

                city_councils_zones = city_councils_zones.set_crs('epsg:6875', allow_override=True)
                # ZMU_ROMA = gpd.read_file(path_app + "static/zmu_roma_index.geojson")

                ## convert into lat , lon
                city_councils_zones = city_councils_zones.to_crs({'init': 'epsg:4326'})
                # ZMU_ROMA = ZMU_ROMA.to_crs(4326)
                city_councils_zones = city_councils_zones.set_crs('epsg:4326', allow_override=True)
                ...
                city_councils_zones = city_councils_zones[['area', 'zmu', 'comune', 'nome_comun', 'quartiere', 'pgtu',
                                                           ...
                                                           'altro', 'add_comm', 'add_spbl', 'add_sprv', 'add_scu',
                                                           'add_uni',
                                                           'add_altri', 'add_tot' , 'geometry']]
                ZMU_ROMA_with_population = gpd.GeoDataFrame(city_councils_zones)

            #ZMU_ROMA_with_population = gpd.read_file(path_app + "static/zones_italy_backend.geojson")
            #ZMU_ROMA_with_population['centroid'] = ZMU_ROMA_with_population.apply(wkb_tranformation_centroid, axis=1)

            if selected_province_number != 58:
                print("--------------- OTHER province than ROME.....")
                ZMU_ROMA_with_population.reset_index(inplace=True)
                ZMU_ROMA_with_population.rename({'index': 'index_zmu'}, axis=1, inplace=True)
                ZMU_ROMA_with_population.rename({'pop_mag15': 'POP_TOT_ZMU'}, axis=1, inplace=True)
            ...
            mean_aggregated_zmu_origin = routes[['index_zmu_origin', 'zmu_origin', 'triptime_s', 'breaktime_s_dest', 'dist', 'speed']].groupby(['index_zmu_origin', 'zmu_origin'],
                                    sort=False).mean().reset_index().rename(
                columns={0: 'means'})
            ...
            mean_aggregated_zmu_origin.rename({'breaktime_s_dest': 'mean_breaktime_m_dest'}, axis=1, inplace=True)
            mean_aggregated_zmu_origin['mean_breaktime_m_dest'] = (mean_aggregated_zmu_origin['mean_breaktime_m_dest']) / 60
            ...
            mean_aggregated_zmu_destination = routes[
                ['index_zmu_destination', 'zmu_destination', 'triptime_s', 'breaktime_s_dest', 'dist', 'speed']].groupby(
                ['index_zmu_destination', 'zmu_destination'],
                sort=False).mean().reset_index().rename(
                columns={0: 'means'})   # 'breaktime_s'
            mean_aggregated_zmu_destination.rename({'triptime_s': 'mean_triptime_m'}, axis=1, inplace=True)
            mean_aggregated_zmu_destination['mean_triptime_m'] = (mean_aggregated_zmu_destination['mean_triptime_m']) / 60
            # mean_aggregated_zmu_destination.rename({'breaktime_s': 'mean_breaktime_m'}, axis=1, inplace=True)
            # mean_aggregated_zmu_destination['mean_breaktime_m'] = (mean_aggregated_zmu_destination['mean_breaktime_m']) / 60
            ...
            if selected_province_number != 99:
                ...
                aggregated_zmu_origin = aggregated_zmu_origin[
                                  ['index_zmu_origin','zmu', 'counts_norm', 'mean_triptime_m', 'mean_breaktime_m_dest', 'mean_speed',
                ...
                aggregated_zmu_origin['ratio_num_orig_dest'] = (aggregated_zmu_origin.counts_norm) / (aggregated_zmu_destination.counts_norm_dest)
                aggregated_zmu_origin = pd.merge(aggregated_zmu_origin, aggregated_zmu_destination[['zmu', 'counts_norm_dest']], on=['zmu'],  how='left')    # 'mean_breaktime_m_dest'

                print("-------- I am HERE .....FCD-------------------------------------------------------------------------------")

                print(aggregated_zmu_origin.columns)

            if selected_province_number == 99:  ### ITALY _EV
                    ## merge trip counts @destination with total staypoints in each traffic ZONE ZMU
                    # aggregated_zmu_origin = pd.merge(aggregated_zmu_origin, all_staypoints,
                    #                                 left_on='zmu_origin', right_on='zmu')
                    aggregated_zmu_origin.rename({'zmu_origin': 'zmu'}, axis=1, inplace=True)
                    aggregated_zmu_origin = pd.merge(aggregated_zmu_origin,
                                                         ZMU_ROMA_with_population[
                                                             ['zmu', 'index_zmu']], on=['zmu'], how='left')

                    aggregated_zmu_origin['counts_norm'] = aggregated_zmu_origin.counts / 1
                    aggregated_zmu_origin['count_staypoints'] = 0

                    aggregated_zmu_origin = aggregated_zmu_origin[
                        ['index_zmu_origin', 'zmu', 'counts_norm', 'mean_triptime_m', 'mean_breaktime_m_dest',
                         'mean_speed',
                         'mean_tripdistance_m', 'count_staypoints']]
                    ...
                    aggregated_zmu_destination = pd.merge(aggregated_zmu_destination, mean_aggregated_zmu_destination,
                                                          ...
                                                          how='left')
                    ## merge trip counts @destination with total staypoints in each traffic ZONE ZMU
                    aggregated_zmu_destination.rename({'zmu_destination': 'zmu'}, axis=1, inplace=True)
                    aggregated_zmu_destination = pd.merge(aggregated_zmu_destination,
                                                     ZMU_ROMA_with_population[
                                                         ['zmu', 'index_zmu']], on=['zmu'], how='left')

                    aggregated_zmu_destination['counts_norm'] = aggregated_zmu_destination.counts / 1
                    aggregated_zmu_destination['count_staypoints'] = 0
                    ...
                    aggregated_zmu_destination = aggregated_zmu_destination[
                        ['index_zmu_destination', 'zmu', 'counts_norm', 'mean_triptime_m', 'mean_breaktime_m_dest',
                         'mean_speed',
                         'mean_tripdistance_m', 'count_staypoints']]

                    # aggregated_zmu_origin.rename({'zmu_origin': 'zmu'}, axis=1, inplace=True)
                    aggregated_zmu_destination['zmu'] = aggregated_zmu_destination.zmu.astype('int')
                    aggregated_zmu_destination = pd.merge(aggregated_zmu_destination, ZMU_ROMA_with_population,
                                                          on=['zmu'],
                                                          how='left')
                    ...
                    aggregated_zmu_origin = pd.merge(aggregated_zmu_origin,
                                                     aggregated_zmu_destination[['zmu', 'counts_norm_dest']],
                                                     on=['zmu'], how='left')  # 'mean_breaktime_m_dest'
            ...
            if selected_province_number != 99:
                ### ---- consider only CROSSED ZONES ------ ##################
                origin_destination_routes_day = origin_destination_routes_day[(origin_destination_routes_day.id_zone_o != origin_destination_routes_day.id_zone_d)]
            ...
            origin_destination_routes_day = pd.merge(origin_destination_routes_day,
                                                     ZMU_ROMA_with_population[
                                                         ['zmu', 'index_zmu', 'area', 'nome_comun', 'quartiere', 'POP_TOT_ZMU']],
                                                     on=['zmu'], how='left')
            ...
            if selected_province_number != 99:
                ...
                homes_and_pop = pd.read_csv(path_app + 'static/fcd_vs_istat_pop.csv')
                homes_and_pop.rename({'istat_pop': 'POP_TOT_ZMU'}, axis=1, inplace=True)
                homes_and_pop.rename({'fcd_home': 'n_homes'}, axis=1, inplace=True)

                homes_and_pop['n_homes'] = (homes_and_pop['n_homes'] * 100) / 0.56

                homes_and_pop = pd.merge(homes_and_pop, ZMU_ROMA_with_population[['zmu', 'index_zmu']], on=['zmu'], how='left')

                homes_and_pop['ratio_homes_pop'] = ((homes_and_pop.n_homes) / (homes_and_pop.POP_TOT_ZMU))*100
                # print(homes_and_pop)

                homes_and_pop['ratio_homes_pop'] = round(homes_and_pop.ratio_homes_pop,ndigits=3)
                # homes_and_pop.replace([np.inf, -np.inf], np.nan, inplace=True)
                homes_and_pop = homes_and_pop[(homes_and_pop.ratio_homes_pop < 100) & (homes_and_pop.ratio_homes_pop > 0)]
            ...
            if selected_province_number == 99:
                ...
                aggregated_zmu_origin_day['count_staypoints'] = 0
                ...
                aggregated_zmu_origin_day.rename({'zmu_origin': 'zmu'}, axis=1, inplace=True)
                aggregated_zmu_origin_day['zmu'] = aggregated_zmu_origin_day.zmu.astype('int')
                aggregated_zmu_origin_day = aggregated_zmu_origin_day[
            ...
            aggregated_means_zmu_origin = origin_destination_routes_day[
                ['zmu', 'hr', 'triptime_s', 'breaktime_s_dest', 'tripdistance_m']].groupby(['hr', 'zmu'],
                                                                                             sort=False).mean().reset_index().rename(
                columns={0: 'means'})
            ...
            aggregated_means_zmu_origin['breaktime_m_dest'] = (aggregated_means_zmu_origin['breaktime_s_dest']) / 60
            aggregated_means_zmu_origin = aggregated_means_zmu_origin[
                aggregated_means_zmu_origin['breaktime_m_dest'].notna()]
            aggregated_means_zmu_origin['breaktime_m_dest'] = aggregated_means_zmu_origin.breaktime_m_dest.astype('int')
            ...
            aggregated_24h_mean = aggregated_means_zmu_origin[
                ['zmu', 'triptime_m', 'breaktime_m_dest', 'tripdistance_m']].groupby(['zmu'],
                                                                                sort=False).mean().reset_index().rename(
                columns={0: 'means'})
            ...
            aggregated_24h_mean.rename({'breaktime_m_dest': 'mean_breaktime_m_dest'}, axis=1, inplace=True)
            ...
            aggregated_zmu_origin = pd.merge(
                aggregated_zmu_origin[['index_zmu_origin', 'zmu', 'index_zmu', 'POP_TOT_ZMU', 'counts_norm',
                                       'area', 'comune', 'nome_comun', 'quartiere', 'pgtu', 'municipio', 'geometry']], aggregated_24h_mean, on=['zmu'], how='left')

            # print("aggregated_zmu_origin_for_geojson----------------------", aggregated_zmu_origin_for_geojson.columns)
            ...
            aggregated_zmu_origin['mean_tripdistance_m'] = round(aggregated_zmu_origin.mean_tripdistance_m,
                                                                   ndigits=2)
            aggregated_zmu_origin['mean_breaktime_m_dest'] = round(aggregated_zmu_origin.mean_breaktime_m_dest,
                                                                   ndigits=2)
            ...
            ## grouby + counts....only if you want to estimate the NUMBER of vehicles
            aggregated_destination_routes_day_copy_dest = origin_destination_routes_day_copy_dest[['hr', 'zmu']].groupby(
                ['hr', 'zmu'], sort=False).size().reset_index().rename(columns={0: 'counts'})

            if selected_province_number != 99:
                ## merge trip counts @destination with total staypoints in each traffic ZONE ZMU
                aggregated_destination_routes_day_copy_dest = pd.merge(aggregated_destination_routes_day_copy_dest, all_staypoints,
                ...
                aggregated_destination_routes_day_copy_dest['counts_norm'] = round(aggregated_destination_routes_day_copy_dest['counts_norm'], 2)
                ...
                aggregated_destination_routes_day_copy_dest['zmu'] = aggregated_destination_routes_day_copy_dest.zmu.astype('int')
                aggregated_destination_routes_day_copy_dest = aggregated_destination_routes_day_copy_dest[
                    ['hr', 'zmu', 'counts', 'count_staypoints', 'counts_norm']]
                aggregated_destination_routes_day_copy_dest = pd.merge(aggregated_destination_routes_day_copy_dest, ZMU_ROMA_with_population,
                                                     on=['zmu'], how='left')
                aggregated_destination_routes_day_copy_dest = aggregated_destination_routes_day_copy_dest[
                    aggregated_destination_routes_day_copy_dest['index_zmu'].notna()]
                aggregated_destination_routes_day_copy_dest['index_zmu'] = aggregated_destination_routes_day_copy_dest.index_zmu.astype('int')
            ...
            if selected_province_number == 99:
                ...
                #                                                       all_staypoints, left_on='zmu', right_on='zmu')
                ...
                aggregated_destination_routes_day_copy_dest['count_staypoints'] = 0
                ...
                aggregated_destination_routes_day_copy_dest = aggregated_destination_routes_day_copy_dest[
                    aggregated_destination_routes_day_copy_dest['zmu'].notna()]
                aggregated_destination_routes_day_copy_dest[
                    'zmu'] = aggregated_destination_routes_day_copy_dest.zmu.astype('int')
            ...
            aggregated_means_zmu_destination['breaktime_m_dest'] = (aggregated_means_zmu_destination['breaktime_s_dest']) / 60
            aggregated_means_zmu_destination = aggregated_means_zmu_destination[
                aggregated_means_zmu_destination['breaktime_m_dest'].notna()]
            aggregated_means_zmu_destination['breaktime_m_dest'] = aggregated_means_zmu_destination.breaktime_m_dest.astype('int')
            ...
            aggregated_means_zmu_destination = pd.merge(aggregated_means_zmu_destination, aggregated_destination_routes_day_copy_dest,
                                                   on=['zmu', 'hr'], how='left')
            ...
            ## compute total counts over ALL ZONES for a given hour-range, and compute total mean FOR ALL ZONEs for a given hour-range
            aggregated_means_zmu_destination.replace([np.inf, -np.inf], np.nan, inplace=True)
            aggregated_means_zmu_destination = aggregated_means_zmu_destination[aggregated_means_zmu_destination['counts_norm'].notna()]
            ...
            aggregated_24h_mean_destination = pd.merge(aggregated_24h_mean_destination, aggragated_24h_counts_destination,
                                           on=['zmu'], how='left')
            ...
            aggregated_zmu_destination = pd.merge(
                aggregated_zmu_destination[['index_zmu_destination', 'zmu', 'index_zmu', 'POP_TOT_ZMU', 'counts_norm_dest',
                                       'area', 'comune', 'nome_comun', 'quartiere', 'pgtu', 'municipio', 'lon',
            ...
            aggregated_zmu_destination = pd.merge(
                aggregated_zmu_destination[
                    ['index_zmu_destination', 'zmu', 'index_zmu', 'POP_TOT_ZMU', 'counts_norm_dest',
                     'area', 'comune', 'nome_comun', 'quartiere', 'pgtu', 'municipio', 'geometry']], aggregated_24h_mean_destination,
                on=['zmu'], how='left')
            ...
            #                                                  ndigits=2)
            aggregated_zmu_destination['mean_breaktime_m_dest'] = round(aggregated_zmu_destination.mean_breaktime_m_dest,
                                                                   ndigits=2)
            ...
            TOTAL_MAX_BREAKTIME = np.max(aggregated_zmu_origin.mean_breaktime_m_dest)
            ...
            aggregated_zmu_destination['radial_counts'] = aggregated_zmu_destination['counts24'] * 100 / TOTAL_TRIPS_COUNTS_destination
            ...
            aggregated_zmu_origin['radial_ratio_trip_counts'] = (aggregated_zmu_origin['radial_counts']) / (aggregated_zmu_destination['radial_counts'])
            ...
            aggregated_zmu_origin = aggregated_zmu_origin[
                aggregated_zmu_origin['mean_breaktime_m_dest'].notna()]
            aggregated_zmu_origin['radial_mean_breaktime'] = (aggregated_zmu_origin['mean_breaktime_m_dest']) * 100 / TOTAL_MAX_BREAKTIME
            aggregated_zmu_origin['radial_mean_breaktime'] = round(aggregated_zmu_origin.mean_breaktime_m_dest, ndigits=2)
            ...
            aggregated_zmu_destination['radial_mean_breaktime_dest'] = round(aggregated_zmu_destination.radial_mean_breaktime_dest,
                                                                   ndigits=2)
            ...
            aggregated_zmu_origin['score_breaktime_dest'] = 0
            ...
            for i in range(len(aggregated_zmu_origin)):
                if (aggregated_zmu_origin.radial_mean_breaktime.iloc[i] >= 0) and (
                    ...
                    aggregated_zmu_origin.score_breaktime_dest.iloc[i] = 1
                    ...
                    aggregated_zmu_origin.score_breaktime_dest.iloc[i] = 2
                    ...
                    aggregated_zmu_origin.score_breaktime_dest.iloc[i] = 3
                    ...
                    aggregated_zmu_origin.score_breaktime_dest.iloc[i] = 4
            ...
            #####----->>> save csv file ---- ##################################################
            csv_aggregated_zmu_origin = aggregated_zmu_origin[[ 'index_zmu_origin', 'zmu','score_trip_counts', 'score_trip_time', 'score_breaktime_dest',
            ...
            csv_aggregated_zmu_origin = pd.merge(csv_aggregated_zmu_origin, aggregated_zmu_destination[[ 'zmu', 'score_breaktime_dest', 'mark_breaktime_dest']],
                                                   on=['zmu'], how='left')


            # print(csv_aggregated_zmu_origin.columns)

            csv_aggregated_zmu_origin.rename({'score_breaktime_dest_x': 'score_breaktime'}, axis=1, inplace=True)
            csv_aggregated_zmu_origin.rename({'score_breaktime_dest_y': 'score_breaktime_dest'}, axis=1, inplace=True)
            ...
            aggregated_means_zmu_origin = aggregated_means_zmu_origin[
                ['counts_norm', 'hr', 'zmu', 'triptime_s', 'tripdistance_m', 'breaktime_m_dest',
            ...
            aggregated_means_zmu_origin.rename({'breaktime_m_dest': 'breaktime_m'}, axis=1, inplace=True)
            ...
            # aggregated_zmu_origin_day.rename({'breaktime_m_dest': 'breaktime_m'}, axis=1, inplace=True)

            aggregated_zmu_origin_day = pd.merge(aggregated_zmu_origin_day,
                     aggregated_means_zmu_destination[['index_zmu', 'zmu', 'hr', 'breaktime_m_dest', 'counts_norm_dest']],
                     on=['index_zmu', 'zmu', 'hr'],
                     how='left')


            ## compute ration between N trips @origin and N trips @ destination
            aggregated_zmu_origin_day['Ratio_N_orig_dest'] = (aggregated_zmu_origin_day.counts_norm)/aggregated_zmu_origin_day.counts_norm_dest
            ...
            aggregated_zmu_origin_for_geojson['counts_norm_dest'] = round(aggregated_zmu_origin_for_geojson.counts_norm_dest, ndigits=2)
            aggregated_zmu_origin_for_geojson['counts_norm'] = round(aggregated_zmu_origin_for_geojson.counts_norm, ndigits=2)
            aggregated_zmu_origin_for_geojson['ratio_num_orig_dest'] = round(aggregated_zmu_origin_for_geojson.ratio_num_orig_dest, ndigits=2)
            # aggregated_zmu_origin_for_geojson['mean_breaktime_m'] = round(aggregated_zmu_origin_for_geojson.mean_breaktime_m, ndigits=2)
            aggregated_zmu_origin_for_geojson['mean_breaktime_m_dest'] = round(aggregated_zmu_origin_for_geojson.mean_breaktime_m_dest, ndigits=2)
            aggregated_zmu_origin_for_geojson['mean_tripdistance_m'] = round(aggregated_zmu_origin_for_geojson.mean_tripdistance_m, ndigits=2)
            aggregated_zmu_origin_for_geojson['mean_triptime_m'] = round(aggregated_zmu_origin_for_geojson.mean_triptime_m, ndigits=2)
            ...
            aggregated_zmu_origin_for_geojson.to_file(filename=path_app + 'static/cache/' + str(selected_data_type) + '_' + str(selected_province_number) + '_'+
                                                               str(selected_indicator_type) + '_' + str(selected_ZMU_day_start) + '_' + str(selected_ZMU_day_end) + '_' +
                                                               str(selected_ZMU_tod) + '_' +  str(selected_stay_type) + '_' + str(selected_ZMU_hourrange) + '_' +
                                                                'aggregated_zmu_filtered.geojson',
                                                      driver='GeoJSON')
            ...
            print("---------------------------------------------------",
                  ...
                  "selected_province_number:", selected_province_number,
                  "selected_indicator_type:", selected_indicator_type,
                  "selected_ZMU_day_start:", selected_ZMU_day_start,
                  "selected_ZMU_day_end:", selected_ZMU_day_end,
                  "selected_ZMU_tod:", selected_ZMU_tod,
                  "selected_stay_type:", selected_stay_type,
                  "selected_ZMU_hourrange", selected_ZMU_hourrange,
                  "---------------------------------------------------")
...
"""
### select COSTS TYPE  -------- #################################
@app.route('/costs_type_selector/', methods=['GET', 'POST'])
def costs_type_selector():
    import glob
    ...
    if request.method == "POST":
        ...
        session["costs_type"] = request.form.get("costs_type")
        selected_costs_type = session["costs_type"]
        print("selected_costs_type:", selected_costs_type)
        selected_costs_type = str(selected_costs_type)

        ## --------- SELECT DATA TYPE and redirect to new .html page -------- ##############
        selected_costs_type = int(selected_costs_type)
        if selected_costs_type == 113:  ## NOX cost
            print("NOx")
            cost = 'NOx_cost'
            print("selected_costs_type:--------selector", selected_costs_type, cost)


        elif selected_costs_type == 114:  ## NMVOC cost
            print("nmvoc_cost")
            cost = 'nmvoc'
            print("selected_costs_type:--------selector", selected_costs_type, cost)

        elif selected_costs_type == 115:  ## PM cost
            print("PM cost")
            cost = 'PM_cost'
            print("selected_costs_type:--------selector", selected_costs_type, cost)

        elif selected_costs_type == 116:  ## PM_nexh cost
            print("PM_nexh cost")
            cost = 'PM_nexh_cost'
            print("selected_costs_type:--------selector", selected_costs_type, cost)

        elif selected_costs_type == 117:  ## CO2 eq cost
            print("CO2eq cost")
            cost = 'CO2_eq_cost'
            print("selected_costs_type:--------selector", selected_costs_type, cost)

        elif selected_costs_type == 118:  ## CO2 eq wtt cost
            print("CO2eq_wtt cost")
            cost = 'CO2_eq_wtt_cost'
            print("selected_costs_type:--------selector", selected_costs_type, cost)


        with open(path_app + "static/params/selected_costs_type_" + session.sid + ".txt", "w") as file:
            file.write(str(selected_costs_type))
...
"""
### select EXTERNALITIES TYPE  -------- #################################
@app.route('/external_cost_type_selector/', methods=['GET', 'POST'])
def external_cost_type_selector():
    import glob

    if request.method == "POST":

        session["external_type"] = request.form.get("external_type")
        selected_external_type = session["external_type"]
        print("selected_external_type:", selected_external_type)
        selected_external_type = str(selected_external_type)
        ...
        with open(path_app + "static/params/selected_costs_type_" + session.sid + ".txt", "r") as file:
            selected_costs_type = file.read()
        print("selected_costs_type-------I AM HERE-----------: ", selected_costs_type)
        session["costs_type"] = selected_costs_type

        ## --------- SELECT DATA TYPE and redirect to new .html page -------- ##############
        selected_external_type = int(selected_external_type)
        if selected_external_type == 211:  ## vo cost
            print("v0 cost")
            external = 'v0_cost'
            print("selected_costs_type:--------selector", selected_external_type, external)


        elif selected_external_type == 212:  ## real cost
            print("real cost")
            cost = 'real_cost'
            print("selected_costs_type:--------selector", selected_external_type, cost)

        elif selected_external_type == 214:  ## deltatime cost
            print("deltatime cost")
            cost = 'deltatime_cost'
            print("selected_costs_type:--------selector", selected_external_type, cost)

        elif selected_external_type == 215:  ## acc cost
            print("acc cost")
            cost = 'acc_cost'
            print("selected_external_type:--------selector", selected_external_type, cost)

        elif selected_external_type == 215:  ## noise cost
            print("noise cost")
            cost = 'noise_cost'
            print("selected_external_type:--------selector", selected_external_type, cost)


        with open(path_app + "static/params/selected_external_type_" + session.sid + ".txt", "w") as file:
            file.write(str(selected_external_type))
...
def fleet_type_selector():
    ...
    if request.method == "POST":
        ...
        with open(path_app + "static/params/selected_external_type_" + session.sid + ".txt", "r") as file:
            selected_external_type = file.read()
        print("selected_external_type-------I AM HERE-----------: ", selected_external_type)
        session["external_type"] = selected_external_type
        """
...
def ZMU_emiss_hourrange_selector():
    ...
    if request.method == "POST":
        ...
        selected_costs_type_files = glob.glob(path_app + "static/params/selected_costs_type_*.txt")
        selected_costs_type_files.sort(key=os.path.getmtime)
        selected_costs_type_file = selected_costs_type_files[len(selected_costs_type_files) - 1]
        with open(selected_costs_type_file) as file:
            selected_costs_type = file.read()
        print("selected_costs_type-------I AM HERE-----------: ", selected_costs_type)



        selected_external_type_files = glob.glob(path_app + "static/params/selected_external_type_*.txt")
        selected_external_type_files.sort(key=os.path.getmtime)
        selected_external_type_file = selected_external_type_files[len(selected_external_type_files) - 1]
        with open(selected_external_type_file) as file:
            selected_external_type = file.read()
        print("selected_external_type-------I AM HERE-----------: ", selected_external_type)
...
@app.route('/modal_page/', methods=['GET', 'POST'])
def modal_page():
    ...
    return render_template("index_modal.html")



@app.route('/modal_selected/', methods=['GET', 'POST'])
def modal_selected():
    if request.method == "POST":

        # selected_modal_pt = '0.85'

        session["modal_pt"] = request.form.get("modal_pt")
        selected_modal_pt = session["modal_pt"]
        print("selected_modal_percentage:", selected_modal_pt)
        selected_modal_pt = str(selected_modal_pt)


        with open(path_app + "static/params/selected_modal_pt_" + session.sid + ".txt", "w") as file:
            file.write(str(selected_modal_pt))

        selected_modal_pt = int(selected_modal_pt)

        #### --- select the modal prcentages
        if selected_modal_pt == 103:
            modal_pt = "0.8"
        elif selected_modal_pt == 104:
            modal_pt = "0.85"
        elif selected_modal_pt == 105:
            modal_pt = "0.9"
        elif selected_modal_pt == 106:
            modal_pt = "0.95"
        ...
        print("---------------------------------------- modal shift percentage selected")

        query_modal_zmu = text('''SELECT *  FROM shift.modal_shift
                                                WHERE delta_pt =:x ''')

        stmt = query_modal_zmu.bindparams(x=str(modal_pt))
        with engine.connect() as conn:
            res = conn.execute(stmt).all()
        df_modal = pd.DataFrame(res)  ## the pair 'home_zone', 'work_zone' are already unique, no need to group


        #### group by "home zone" ------- #################
        aggregated_modal = df_modal[['home_zone', 'shift_pr']].groupby(['home_zone'], sort=False).sum().reset_index()
        aggregated_modal['shift_pr'] = round(aggregated_modal['shift_pr'], 4)

        ### ----- >> merge with zmu zones #################
        aggregated_modal.rename({'home_zone': 'zmu'}, axis=1, inplace=True)

        aggregated_modal = pd.merge(aggregated_modal, ZMU_ROMA, on=['zmu'], how='left')
        aggregated_modal = aggregated_modal[aggregated_modal['geometry'].notna()]
        aggregated_modal = aggregated_modal[['zmu', 'shift_pr', 'index_zmu', 'POP_TOT_ZMU',
                                                'nome_comun', 'quartiere', 'geometry']]
        aggregated_modal.shift_pr = round(aggregated_modal.shift_pr, ndigits=4)



        df_modal.rename({'home_zone': 'zmu'}, axis=1, inplace=True)
        df_modal = pd.merge(df_modal, ZMU_ROMA, on=['zmu'], how='left')
        df_modal = df_modal[df_modal['geometry'].notna()]
        df_modal = df_modal[['zmu', 'shift_pr', 'index_zmu', 'work_zone', 'POP_TOT_ZMU',
                             'nome_comun', 'quartiere', 'geometry']]
        df_modal.shift_pr = round(df_modal.shift_pr, ndigits=4)
        ## save data
        df_modal.to_csv(path_app + 'static/df_modal_work_' + session.sid + '.csv')
        # df_modal.to_csv(path_app + 'static/df_modal_work.csv')

        try:
            aggregated_modal = gpd.GeoDataFrame(aggregated_modal)
        except IndexError:
            abort(404)

        ## save as .geojson file
        aggregated_modal.to_file(filename=path_app + 'static/aggregated_modal_work.geojson',
                                        driver='GeoJSON')
        ...
        ### convert Geodataframe into .geojson...
        session["aggregated_modal_work"] = aggregated_modal.to_json()
        print("------ I am here----- SESSION Colored ZMUs--------------------------")

        return render_template("index_modal_selected.html",
                               session_aggregated_modal_work = session["aggregated_modal_work"])





@app.route('/choose_zmu_index_modal_shift_static', methods=['POST'])
def choose_zmu_index_modal_shift_static():
    data = request.json['data']
    session["ZMU_index"] = data
    print("I am here...selected_index_zmu is: ", session["ZMU_index"])
    print("-------------- I am here...selected_index_zmu is:---------------------- ", session["ZMU_index"])
...
@app.route('/choose_zmu_index_modal_shift', methods=['POST'])
def choose_zmu_index_modal_shift():
    data = request.json['data']
    session["ZMU_index"] = data
    print("I am here...selected_index_zmu is: ", session["ZMU_index"])

    #### ----- relaod modal shift data matrix
    df_modal = pd.read_csv(path_app + 'static/df_modal_work_' + session.sid + '.csv')
    # df_modal = pd.read_csv(path_app + 'static/df_modal_work_de8b9a6f-ce79-4600-a529-4aa45ed9eab4.csv')
    print("I am here...selected_index_zmu is: ", session["ZMU_index"])
    print(len(df_modal))

    ZMU_ROMA = gpd.read_file(path_app + "static/zmu_aggregated_POP_ROMA_2015_new.geojson")

    df_modal['index_zmu'] = df_modal['index_zmu'].astype(int)

    selected_index_zmu = session["ZMU_index"]
    selected_index_zmu = int(float(selected_index_zmu))
    # selected_index_zmu = 18
    print("--------------------selected_index_zmu;--------------------------", selected_index_zmu)
    ## select ORIGIN zone ZMU .......
    filtered_od_modal = df_modal[
        df_modal.index_zmu == selected_index_zmu]  ## from home ---> to work
    filtered_od_modal.reset_index(inplace=True)
    input_zmu = int(filtered_od_modal.zmu[0])
    # input_index_zmu = filtered_od_modal.index_zmu[1]
    print("----------------------input_zmu:---------------------------------------------------------------->>>>>>>>",
          input_zmu)

    filtered_od_modal = filtered_od_modal[['work_zone', 'shift_pr']]
    filtered_od_modal.rename({'work_zone': 'zmu'}, axis=1, inplace=True)
    # AAA = filtered_od_routing[filtered_od_routing.zmu == '1002019']
    ...
    filtered_od_modal = pd.concat([starting_zone, filtered_od_modal])

    print(
        "----------------------length filtered_od_modal:--------------------->>>>>>>>",
        len(filtered_od_modal))
    ### merge with zmu and build a .json file
    filtered_od_modal = pd.merge(filtered_od_modal, ZMU_ROMA, on=['zmu'], how='left')
    filtered_od_modal.replace([np.inf, -np.inf], np.nan, inplace=True)
    filtered_od_modal.replace(to_replace=[None], value=np.nan, inplace=True)
    filtered_od_modal.drop(columns=['centroid'], inplace=True)
    filtered_od_modal.shift_pr = round(filtered_od_modal.shift_pr, ndigits=4)
    ## save as .geojson file
    try:
        filtered_od_modal = gpd.GeoDataFrame(filtered_od_modal)
    except IndexError:
        abort(404)

    print("----- I AM HERE ------------------------")

    filtered_od_modal.to_file(filename=path_app + 'static/filtered_od_modal_' + session.sid + '.geojson',
                                driver='GeoJSON')
...
#### ---->>> this is to refresh the page....with the FINAL LAYER
@app.route('/redirect_zmu_paths_modal/', methods=['GET', 'POST'])
def redirect_zmu_paths_modal():
    import os

    filtered_od_modal = open(path_app + "static/filtered_od_modal_" + session.sid + ".geojson", )
    filtered_od_modal = json.load(filtered_od_modal)

    session["filtered_od_modal"] = filtered_od_modal
    print("-------------- I am here...------------redirect.....")

    return render_template("index_showing_zmu_modal_redirect.html",
                           session_filtered_od_modal=session["filtered_od_modal"])


@app.route('/layer_ZMU_modal/', methods=['GET', 'POST'])
def layer_ZMU_modal():
    return render_template("index_showing_zmu_modal.html")
...
def TRIP_LEGS_hour_selector():
    if request.method == "POST":
        ...
        """
        session["ZMU_hour"] = request.form["ZMU_hour"]
        print("selected_ZMU_hour....:", session["ZMU_hour"])
        selected_ZMU_hour = str(session["ZMU_hour"])
        """
        ...
        if selected_data_type == 11:  ## FCD data ##
            ...
            query_crossed_zmu = text('''SELECT  dt_o, id_zone_o, id_zone_d, id
                                                                FROM synt.trips_new
                                                                WHERE date(synt.trips_new.dt_o) BETWEEN :x AND :xx 
                                                                 AND id_zone_d != id_zone_o
                                                                 AND synt.trips_new.hour_range_o = :y ''')



        stmt = query_crossed_zmu.bindparams(x=str(selected_ZMU_day_start), xx=str(selected_ZMU_day_end), y=str(hourrange_id))
        with engine.connect() as conn:
            res = conn.execute(stmt).all()
        crossed_zmu = pd.DataFrame(res)

        crossed_zmu = crossed_zmu.sample(frac=0.99)  # 40%;
        ...
        grouped_trajectories_ORIGIN = pd.merge(grouped_trajectories, ZMU_ROMA[['zmu', 'POP_TOT_ZMU', 'geometry', 'centroid']],
                                               left_on='zmu_origin', right_on='zmu', how='left')
        grouped_trajectories_DESTINATION = pd.merge(grouped_trajectories, ZMU_ROMA[['zmu', 'POP_TOT_ZMU', 'geometry', 'centroid']],
        ...
        grouped_trajectories = pd.merge(grouped_trajectories,
                                               grouped_trajectories_ORIGIN[['zmu_origin', 'zmu_destination', 'centroid']],how='left')
        ...
        grouped_trajectories_DESTINATION = grouped_trajectories_DESTINATION[grouped_trajectories_DESTINATION['centroid'].notna()]
        ...
        if selected_data_type == 11:  ## FCD data ##
            ...
            grouped_trajectories = grouped_trajectories.sample(frac=0.99)  # 40%;
...
def AGGREGATERD_ZMU_border_selector():
    ...
    if request.method == "POST":
        ...
        if selected_data_type == 11:       ### FCD data
            ...
            origin_routes_day = origin_routes_day.sample(frac=0.1)  # 10%; # n=1000
            ...
            destination_routes_day = destination_routes_day.sample(frac=0.1)  # 50%
            ...
            destination_routes_day = destination_routes_day[['p_after', 'tt', 'dt_d', 'id', 'dist', 'hr', 'day']]
            ...
            destination_routes_day.rename({'p_after': 'breaktime_s'}, axis=1, inplace=True)
            ...
            origin_routes_day['geom'] = origin_routes_day.apply(wkb_tranformation, axis=1)
            origin_routes_day = gpd.GeoDataFrame(origin_routes_day)
            origin_routes_day.rename({'geom': 'geometry'}, axis=1, inplace=True)

            ## reference system = 6875 (in meters)
            origin_routes_day = origin_routes_day.set_geometry("geometry")
            origin_routes_day = origin_routes_day.set_crs('epsg:6875', allow_override=True)

            ## convert into lat , lon
            origin_routes_day = origin_routes_day.to_crs({'init': 'epsg:4326'})
            origin_routes_day = origin_routes_day.set_crs('epsg:4326', allow_override=True)

            origin_routes_day = origin_routes_day.sample(frac=0.2)  # 50%
            ## get the last coordinate point....LONG RUN.....
            orig_lats = []
            orig_lons = []
            for i in range(len(origin_routes_day)):
                row = origin_routes_day.geometry.iloc[i]
                x, y = row.coords.xy
                lats_longs = pd.DataFrame(list(zip(x, y)), columns=['longitude', 'latitude'])
                origin_longitude = lats_longs.iloc[len(lats_longs) - 1][0]
                origin_latitude = lats_longs.iloc[len(lats_longs) - 1][1]
                orig_lons.append(origin_longitude)
                orig_lats.append(origin_latitude)

            origin_routes_day = origin_routes_day[['p_before', 'tt', 'dt_o', 'id', 'dist', 'hr', 'day']]
            origin_routes_day['longitude'] = orig_lons
            origin_routes_day['latitude'] = orig_lats

            origin_routes_day.rename({'tt': 'triptime_s'}, axis=1, inplace=True)
            origin_routes_day.rename({'p_before': 'breaktime_s_origin'}, axis=1, inplace=True)
            origin_routes_day.rename({'id': 'idtrajectory'}, axis=1, inplace=True)
            origin_routes_day.rename({'dist': 'tripdistance_m'}, axis=1, inplace=True)

            geometry_o = [Point(xy) for xy in zip(origin_routes_day.longitude, origin_routes_day.latitude)]
            geo_origin_routes_day = GeoDataFrame(origin_routes_day, geometry=geometry_o,
                                                      crs="EPSG:4326")

            # get ZMU containing the DESTINATION locations (gdf_tdf)
            BORDER_origin_routes = gpd.sjoin(BORDER_ZMUS, geo_origin_routes_day, how='right',
                                                  predicate='contains')  ### <---- OK
            BORDER_origin_routes = pd.DataFrame(BORDER_origin_routes)
            ## only get trips ending within the BORDER_ZMUs that is index_left == 0)
            # BORDER_origin_routes = BORDER_origin_routes[BORDER_origin_routes.index_left == 0]

            ## ---->> grouby + counts....only if you want to estimate the NUMBER of vehicles
            aggregated_BORDER_origin_day = BORDER_origin_routes[['hr']].groupby(['hr'],
                                                                            sort=False).size().reset_index().rename(columns={0: 'counts'})

            # aggregated_BORDER_destinations_day['day'] = selected_ZMU_day

            BORDER_origin_routes = BORDER_origin_routes[BORDER_origin_routes['triptime_s'] > 0]
            BORDER_origin_routes = BORDER_origin_routes[BORDER_origin_routes['breaktime_s_origin'] > 0]
            BORDER_origin_routes = BORDER_origin_routes[
                BORDER_origin_routes['tripdistance_m'] > 0]

            #### ------->>>> groupby + mean triptime, tripdistance, breaktime <<<-----------###########################################
            aggregated_means_BORDER_origin = BORDER_origin_routes[
                ['hr', 'triptime_s', 'breaktime_s_origin', 'tripdistance_m']].groupby(['hr'],
                                                                               sort=False).mean().reset_index().rename(columns={0: 'means'})

            # aggregated_means_BORDER_destination['day'] = selected_ZMU_day
            ## convert triptime_s into minutes
            aggregated_means_BORDER_origin['triptime_m'] = (aggregated_means_BORDER_origin['triptime_s']) / 60
            aggregated_means_BORDER_origin = aggregated_means_BORDER_origin[
                aggregated_means_BORDER_origin['triptime_s'].notna()]
            aggregated_means_BORDER_origin['triptime_m'] = aggregated_means_BORDER_origin.triptime_m.astype('int')
            ## convert breaktime_s into minutes
            aggregated_means_BORDER_origin['breaktime_m_origin'] = (aggregated_means_BORDER_origin['breaktime_s_origin']) / 60
            aggregated_means_BORDER_origin = aggregated_means_BORDER_origin[
                aggregated_means_BORDER_origin['breaktime_m_origin'].notna()]
            aggregated_means_BORDER_origin['breaktime_m_origin'] = aggregated_means_BORDER_origin.breaktime_m_origin.astype(
                'int')
            ## convert tripdistance_m
            aggregated_means_BORDER_origin = aggregated_means_BORDER_origin[
                aggregated_means_BORDER_origin['tripdistance_m'].notna()]
            aggregated_means_BORDER_origin[
                'tripdistance_m'] = aggregated_means_BORDER_origin.tripdistance_m.astype('int')

            aggregated_means_BORDER_origin = aggregated_means_BORDER_origin[
                ['hr', 'tripdistance_m', 'triptime_m', 'breaktime_m_origin']]

            ### -----> merge all data together ---- ###################################################################
            aggregated_means_BORDER_origin = pd.merge(aggregated_means_BORDER_origin,
                                                           aggregated_BORDER_origin_day, on=['hr'], how='left')

            # print(aggregated_means_BORDER_origin.columns)
            # print(aggregated_means_BORDER_destination.columns)
            aggregated_means_BORDER_origin.rename({'counts': 'counts_origin'}, axis=1, inplace=True)
            aggregated_means_BORDER_destination.rename({'counts': 'counts_destination'}, axis=1, inplace=True)
            aggregated_means_BORDER_destination.rename({'breaktime_m': 'breaktime_m_destination'}, axis=1, inplace=True)

            aggregated_means_BORDER_OD = pd.merge(aggregated_means_BORDER_origin[['hr', 'tripdistance_m', 'triptime_m', 'counts_origin']],
                                                      aggregated_means_BORDER_destination[['hr', 'breaktime_m_destination', 'counts_destination']], on=['hr'], how='left')

            aggregated_means_BORDER_OD['ratio_trips_origin_destination'] = (aggregated_means_BORDER_OD.counts_origin)/ (aggregated_means_BORDER_OD.counts_destination)

            aggregated_means_BORDER_OD = aggregated_means_BORDER_OD[aggregated_means_BORDER_OD['breaktime_m_destination'].notna()]
            aggregated_means_BORDER_OD = aggregated_means_BORDER_OD[
                aggregated_means_BORDER_OD['ratio_trips_origin_destination'].notna()]

            aggregated_means_BORDER_OD.ratio_trips_origin_destination = round(aggregated_means_BORDER_OD.ratio_trips_origin_destination, ndigits=2)
            ...
            """
            query_destination_day = text('''SELECT   synt.trips.id_veh, synt.trips.p_after, 
                                                                synt.trips.tt, synt.trips.dt_d, synt.trips.id, 
                                                                synt.trips.dist, synt.trips.id_zone_o, synt.trips.id_zone_d,
                                                                           date_part('hour', dt_d) as hr,  synt.trips.geom,
                                                                           TO_CHAR(dt_d::DATE, 'dd-mm-yyyy') as day,
                                                                           synt.staypoints.id_stay_type

                                                                           FROM synt.staypoints
                                                                           INNER JOIN 
                                                                           synt.trips ON synt.trips.id_veh = synt.staypoints.id_veh

                                                                           WHERE date(synt.trips.dt_d) BETWEEN :x AND :xx 
                                                                           AND synt.trips.tod_d = :z
                                                                           /*AND synt.staypoints.id_stay_type != 'O' */
                                                                           AND (synt.staypoints.info->>'virtual')::boolean IS false  
                                                                            ''')
            """
            ...
            """
            query_origin_day = text('''SELECT   synt.trips.id_veh, synt.trips.p_before, 
                                                                            synt.trips.tt, synt.trips.dt_o, synt.trips.id, 
                                                                            synt.trips.dist, synt.trips.id_zone_o, synt.trips.id_zone_d,
                                                                                       date_part('hour', dt_o) as hr,  synt.trips.geom,
                                                                                       TO_CHAR(dt_o::DATE, 'dd-mm-yyyy') as day,
                                                                                       synt.staypoints.id_stay_type
                                                                                       ...
                                                                                       WHERE date(synt.trips.dt_o) BETWEEN :x AND :xx 
            ...
            query_origin_day = text('''SELECT   synt.trips.id_veh, synt.trips.p_before, synt.trips.tt, synt.trips.dt_o, synt.trips.id, 
                                                                                     synt.trips.dist, synt.trips.id_zone_o, synt.trips.id_zone_d,
                                                                                                date_part('hour', dt_o) as hr,  synt.trips.geom,
                                                                                                TO_CHAR(dt_o::DATE, 'dd-mm-yyyy') as day                                            
                                                                                                FROM synt.trips
            ...
            destination_routes_day = destination_routes_day.sample(frac=0.05)  # 10%
            origin_routes_day = origin_routes_day.sample(frac=0.05)  # 10%
            print("---- I am here ----*****----***************--------------")
            ...
            aggregated_means_BORDER_destination = aggregated_means_BORDER_destination[
                aggregated_means_BORDER_destination['triptime_s'].notna()]
            aggregated_means_BORDER_destination['triptime_m'] = aggregated_means_BORDER_destination.triptime_m.astype(
                'int')
            ## convert breaktime_s into minutes
            aggregated_means_BORDER_destination['breaktime_m'] = (aggregated_means_BORDER_destination[
                'breaktime_s']) / 60
            aggregated_means_BORDER_destination = aggregated_means_BORDER_destination[
                aggregated_means_BORDER_destination['breaktime_m'].notna()]
            aggregated_means_BORDER_destination['breaktime_m'] = aggregated_means_BORDER_destination.breaktime_m.astype(
            ...
            aggregated_means_BORDER_origin['triptime_m'] = aggregated_means_BORDER_origin.triptime_m.astype('int')
            ## convert breaktime_s into minutes
            aggregated_means_BORDER_origin['breaktime_m_origin'] = (aggregated_means_BORDER_origin[
                'breaktime_s_origin']) / 60
            aggregated_means_BORDER_origin = aggregated_means_BORDER_origin[
                aggregated_means_BORDER_origin['breaktime_m_origin'].notna()]
            aggregated_means_BORDER_origin[
                'breaktime_m_origin'] = aggregated_means_BORDER_origin.breaktime_m_origin.astype(
                'int')
            ## convert tripdistance_m
            aggregated_means_BORDER_origin = aggregated_means_BORDER_origin[
                aggregated_means_BORDER_origin['tripdistance_m'].notna()]
            ...
            aggregated_means_BORDER_OD = pd.merge(
                aggregated_means_BORDER_origin[['hr', 'tripdistance_m', 'triptime_m', 'counts_origin']],
                aggregated_means_BORDER_destination[['hr', 'breaktime_m_destination', 'counts_destination']], on=['hr'],
                how='left')

            aggregated_means_BORDER_OD['ratio_trips_origin_destination'] = (
                                                                               aggregated_means_BORDER_OD.counts_origin) / (
                                                                               aggregated_means_BORDER_OD.counts_destination)

            aggregated_means_BORDER_OD = aggregated_means_BORDER_OD[
                aggregated_means_BORDER_OD['breaktime_m_destination'].notna()]
            aggregated_means_BORDER_OD = aggregated_means_BORDER_OD[
                aggregated_means_BORDER_OD['ratio_trips_origin_destination'].notna()]

            aggregated_means_BORDER_OD.ratio_trips_origin_destination = round(
                aggregated_means_BORDER_OD.ratio_trips_origin_destination, ndigits=2)



        #####----->>> save csv file ---- ##################################################
        aggregated_means_BORDER_OD.to_csv(
...
##--->> timeseries NUMBER OF VEHICLES @ destination ------ ##############
@app.route('/figure1')
def figure_plotly1():
    import plotly.express as px
    ##---->>> make time series plotly.....-------------#######################
    ...
    filtered_ZMU = filtered_ZMU[['hr', 'counts_norm', 'counts_norm_dest', 'index_zmu', 'tripdistance_m', 'Ratio_N_orig_dest', 'breaktime_m_dest',
                                 'triptime_m', 'breaktime_m']]
    filtered_ZMU.sort_values('hr', ascending=True, inplace=True)
    ...
    fig = px.line(filtered_ZMU, x='hr', y="Ratio_N_orig_dest", labels={
        "Ratio_N_orig_dest": "N.orig / N.dest"})
    ...
    import os
    import glob
    files_to_remove = glob.glob(path_app + 'templates/plotly_plots_counts_destination/*')
    for f in files_to_remove:
        os.remove(f)
    import random
    x = random.randint(0, 10000)
    fig.write_html(path_app + 'templates/plotly_plots_counts_destination/hourly_' + str(x) + '_zmu.html')
    return render_template('plotly_plots_counts_destination/hourly_' + str(x) + '_zmu.html')
...
def figure_plotly2():
    ...
    filtered_ZMU = aggregated_zmu_origin_by_day[
        (aggregated_zmu_origin_by_day.index_zmu == selected_index_zmu)]
    filtered_ZMU = filtered_ZMU[['hr', 'counts_norm', 'index_zmu', 'tripdistance_m', 'Ratio_N_orig_dest', 'breaktime_m_dest',
                                 'triptime_m', 'breaktime_m']]
    filtered_ZMU.sort_values('hr', ascending=True, inplace=True)
    ...
    fig = px.line(filtered_ZMU, x='hr', y="tripdistance_m", labels={
        "tripdistance_m": "trip dist.@Orig (meters)"})
    fig.update_layout(
      plot_bgcolor='white')
...
def figure_plotly4():
    ...
    ## plot houlry profile with plotly and make an. html file
    ##----->>> mean TRIPTIME (minutes) at destination-----#############################
    fig = px.line(filtered_ZMU, x='hr', y="breaktime_m_dest", labels={
        "breaktime_m_dest": "stop time @Dest. (min.)"})
...
def figure_polar_FCD():
    ...
    csv_aggregated_zmu_origin = pd.read_csv(
        path_app + 'static/csv_aggregated_zmu_origin_' + session.sid + '.csv')
    # csv_aggregated_zmu_destinations = pd.read_csv(
    #    path_app + 'static/csv_aggregated_zmu_destinations_f828b521-6dd2-4c01-9dcc-b7463f73061c.csv')
    ...
    aggregated_zmu_origin_selected = csv_aggregated_zmu_origin[['score_trip_counts', 'score_trip_time',
                                                                            ...
                                                                            'mark_breaktime', 'mark_trip_distance', 'score_ratio_trip_counts', 'mark_ratio_trip_counts',
                                                                             'score_breaktime_dest', 'mark_breaktime_dest']][csv_aggregated_zmu_origin.index_zmu == selected_index_zmu]

    aggregated_zmu_origin_selected.reset_index(inplace=True)
    print(aggregated_zmu_origin_selected.columns)
    print(aggregated_zmu_origin_selected)
    print(list(aggregated_zmu_origin_selected[['score_ratio_trip_counts', 'score_trip_time','score_breaktime_dest','score_trip_distance']].iloc[0]))
    print("----- I am here----------------------")
    
    df = pd.DataFrame({'category': ['N.Trips(Orig/Dest)', 'Trip time @Orig', 'Break time @Dest', 'Trip Distance @Orig'],
                       'value': list(aggregated_zmu_origin_selected[['score_ratio_trip_counts', 'score_trip_time',
                                                                           'score_breaktime_dest',
                                                                           'score_trip_distance']].iloc[0]),
                       'mark': list(aggregated_zmu_origin_selected[['mark_ratio_trip_counts', 'mark_trip_time',
                                                                          'mark_breaktime_dest', 'mark_trip_distance']].iloc[0])})
    ...
    # fig.write_html(path_app + 'templates/plotly_polar_fcd/prova_radar_zone_bis.html')
...
def figure_plotly5():
    ...
    aggregated_origin_OD_hourly_profile = pd.read_csv(
        path_app + 'static/origin_OD_hourly_profile_' + session.sid + '.csv')
    # aggregated_origin_OD_hourly_profile = pd.read_csv(path_app + 'static/origin_OD_hourly_profile_a0ebfca3-fb76-44f9-a6c4-710b5601e377.csv')
    ...
    ####---->>> normalize data
    # filtered_ZMU['triptime_m'] = ((filtered_ZMU.triptime_m) * (filtered_ZMU.counts)) / max(filtered_ZMU.counts)
    # filtered_ZMU['tripdistance_m'] = (filtered_ZMU.tripdistance_m * filtered_ZMU.counts) / max(filtered_ZMU.counts)
    # filtered_ZMU['breaktime_m'] = (filtered_ZMU.breaktime_m * filtered_ZMU.counts) / max(filtered_ZMU.counts)
...
def figure_BORDER_plotly1():
    ...
    ####---->>> normalize data
    aggregated_BORDER_destinations_by_day['triptime_m'] = ((aggregated_BORDER_destinations_by_day.triptime_m) * (aggregated_BORDER_destinations_by_day.counts_origin)) / max(
        aggregated_BORDER_destinations_by_day.counts_origin)
    aggregated_BORDER_destinations_by_day['tripdistance_m'] = (aggregated_BORDER_destinations_by_day.tripdistance_m * aggregated_BORDER_destinations_by_day.counts_origin) / max(
        aggregated_BORDER_destinations_by_day.counts_origin)


    ## plot houlry profile with plotly and make an. html file
    ##----->>> number of Vehicles at destination-----#############################
    fig = px.line(aggregated_BORDER_destinations_by_day, x='hr', y="ratio_trips_origin_destination", labels={
        "ratio_trips_origin_destination": "Trips Ratio (Orig/Dest)"})
...
def figure_BORDER_plotly4():
    ...
    ## plot houlry profile with plotly and make an. html file
    ##----->>> number of Vehicles at destination-----#############################
    fig = px.line(aggregated_BORDER_destinations_by_day, x='hr', y="breaktime_m_destination", labels={
        "breaktime_m_destination": "stop time @Dest (min.)"})
...
def animation_FCD():
    ...
    session["FCD_day_start"]  = '2024-10-18'
    session["FCD_day_end"]  = '2024-10-20'
    session["FCD_tod"] = 6
    session["FCD_hourrange"] = 2
    session['input_data_range'] = "10/13/2024 - 10/14/204"
    # session["lat_ORIGIN_province"] = 41.8946
    # session["lon_ORIGIN_province"] = 12.4873
...
def FCD_hour_selector():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            selected_FCD_day_start = '2024-10-04'
            session["FCD_day_start"] = selected_FCD_day_start
            print("using stored variable FCD_day_start: ", session["FCD_day_start"])
        ...
        try:
            ...
            selected_FCD_day_end = '2024-10-15'
            session["FCD_day_end"] = selected_FCD_day_end
        ...
        if selected_province_number == 99:
            ...
            routes_origin_destination.rename({'tripdistance_m': 'dist'}, axis=1, inplace=True)
        ...
        if selected_province_number == 99:
            ...
            routes_origin_destination = routes_origin_destination.sample(frac=0.03)  # 10%; # n=1000
            print("--------------- province: italia_ev")
            #### ---- CITY COUNCILS  IN ITALY ----- ################################################################
            ## get ZMU zones
            city_councils_zones = text('''SELECT * from landuse.zones''')
            ...
            city_councils_zones = city_councils_zones[['area', 'zmu', 'comune', 'nome_comun', 'quartiere', 'pgtu',
                                                       ...
                                                       'add_uni',
                                                       'add_altri', 'add_tot', 'geometry']]
            ZMU_ROMA = gpd.GeoDataFrame(city_councils_zones)
            ZMU_ROMA.reset_index(inplace=True)
        ...
        ##merge with zmu zones
        # print("-------------------------****** I  AM HERE ****************************")
        # print(routes_origin_destination.columns)
        # print(ZMU_ROMA.columns)
        routes_origin_destination = pd.merge(routes_origin_destination,
        ...
        for idx_day, day_id in enumerate(all_days_IDs):
            ...
            for idx, idtrajectory_id in enumerate(all_idtrajectory_IDs):
                ...
                # OD_lista = [((round(trip_polyline[0][0],3)) , (round(trip_polyline[0][1],3))) ,
                #            ((round(trip_polyline[len(trip_polyline) - 1][0],3)) , (round(trip_polyline[len(trip_polyline) - 1][1],3)))]
                ##----> MAKE a dictionary...of points of polyline
                lista = []
...
"""
@app.route('/animation_GTFS/', methods=['GET', 'POST'])
def animation_GTFS():
    session["GTFS_type"] = 11
    session['GTFS_hour'] = ''
    session["GTFS_tod"] = 6
...
def GTFS_LEGS_hour_selector():
    ...
    try:
        ...
        with open(stored_tod_file) as file:
            selected_GTFS_tod = file.read()
        print("stored_tod_file-------I AM HERE-----------: ", selected_GTFS_tod)
    ...
    if request.method == "POST":
        ...
        if (selected_GTFS_type_name == 'bus') & (tod == 'W'):
            ...
            query_gtfs = text('''SELECT route_id, trip_id, from_stop_id, to_stop_id, from_stop_name, from_ctime, 
                                                           ...
                                                           TO_CHAR(from_ctime::DATE, 'dd-mm-yyyy') as day
                                                           FROM gtfs.rail_consumption_feriale
                                                               WHERE extract(hour from from_ctime) = :y
                                                                         /*limit 1000*/ ''')
            print("------GTFS_type------------###########:", selected_GTFS_type_name)
            print("------tod------------########:", tod)
            ...
            query_gtfs = text('''SELECT route_id, trip_id, from_stop_id, to_stop_id, from_stop_name, from_ctime, 
                                                           ...
                                                           date_part('hour', from_ctime) as hour, geom, kwh,
                                                           TO_CHAR(from_ctime::DATE, 'dd-mm-yyyy') as day
                                                           FROM gtfs.rail_consumption_festivo
                                                               WHERE extract(hour from from_ctime) = :y
                                                                         /*limit 1000*/ ''')
            print("------GTFS_type------------###########:", selected_GTFS_type_name)
            print("------tod------------########:", tod)
            ...
            query_gtfs = text('''SELECT route_id, trip_id, from_stop_id, to_stop_id, from_stop_name, from_ctime, 
                                                                  ...
                                                                  date_part('hour', from_ctime) as hour, geom, kwh,
                                                                  TO_CHAR(from_ctime::DATE, 'dd-mm-yyyy') as day
                                                                  FROM gtfs.rail_consumption_prefestivo
                                                                     WHERE extract(hour from from_ctime) = :y
                                                                               /*limit 1000*/ ''')
            print("------GTFS_type------------###########:", selected_GTFS_type_name)
            print("------tod------------########:", tod)
        ...
        merged_bus = gpd.sjoin(gdf_stop_buses, cell, how='left', predicate='within')
        ...
        gtfs_LEGS_all['consumption'] = round(gtfs_LEGS_all['consumption'], ndigits=2)
...
def tpl_emissions_legs_hour_selector():
    ...
    query_veh_type = text('''SELECT * FROM gen.effective_bus_types
                                      WHERE engine = :x AND
                                      segment =:y AND
                                      euro_standard =:z AND
                                      primary_fuel =:xx  ''')
    ...
    if request.method == "POST":
        ...
        """
        ## find the extents of the map
        xmin, ymin, xmax, ymax = gdf_stop_emiss_buses.total_bounds
        # cell_size = 0.005 ## 500 meters
        cell_size = 0.01  ## 1 km (0.01)
        grid_cells = []
        for x0 in np.arange(xmin, xmax + cell_size, cell_size):
            for y0 in np.arange(ymin, ymax + cell_size, cell_size):
                ...
                grid_cells.append(shapely.geometry.box(x0, y0, x1, y1))
        cell = gpd.GeoDataFrame(grid_cells, columns=['geometry'],
                                crs=gdf_stop_emiss_buses.crs)

        merged_bus = gpd.sjoin(gdf_stop_emiss_buses, cell, how='left', op='within')
        ...
        #### send data direclty to the .html file....within tis session
        # session["stop_emiss_buses_heatmap"] = joined_lista
        # print(session["stop_emiss_buses_heatmap"])


        ## save file
        with open(path_app + "static/gridded_emissions_n_buses.txt", "w") as file:
            file.write(str(joined_lista))
        with open(path_app + "static/gridded_emissions_n_buses.txt", "r+") as f:
            old = f.read()  # read everything in the file
            f.seek(0)  # rewind
            f.write("var n_buses = \n" + old)  # assign the "var name" in the .geojson file
        ...
        ###############################################################################
        #### ------------------------------ ###########################################
        #### ---------------- HEAT MAPS for emissisons ----------- ####################
        ###############################################################################

        '''
        ##----  find the extents of the map    ---- ####################
        xmin, ymin, xmax, ymax = gdf_stop_sum_emiss_buses.total_bounds
        # cell_size = 0.005 ## 500 meters
        cell_size = 0.01  ## 1 km (0.01)
        grid_cells = []
        for x0 in np.arange(xmin, xmax + cell_size, cell_size):
            for y0 in np.arange(ymin, ymax + cell_size, cell_size):
                ...
                grid_cells.append(shapely.geometry.box(x0, y0, x1, y1))
        cell = gpd.GeoDataFrame(grid_cells, columns=['geometry'],
                                crs=gdf_stop_sum_emiss_buses.crs)


        if (selected_emission_type == 11):
            merged_emiss_bus = gpd.sjoin(gdf_stop_sum_emiss_buses[['to_lat', 'to_lon', 'to_stop_name', 'ec','geometry']], cell, how='left', op='within')
            #### ----- rename ----- #####
            merged_emiss_bus.rename({'ec': 'emiss'}, axis=1, inplace=True)


        elif (selected_emission_type == 12):
            merged_emiss_bus = gpd.sjoin(
                gdf_stop_sum_emiss_buses[['to_lat', 'to_lon', 'to_stop_name', 'ec_wtt', 'geometry']], cell, how='left',
                op='within')
            merged_emiss_bus.rename({'ec_wtt': 'emiss'}, axis=1, inplace=True)

        elif (selected_emission_type == 13):
            merged_emiss_bus = gpd.sjoin(
                gdf_stop_sum_emiss_buses[['to_lat', 'to_lon', 'to_stop_name', 'nox', 'geometry']], cell, how='left',
                op='within')
            merged_emiss_bus.rename({'nox': 'emiss'}, axis=1, inplace=True)

        elif (selected_emission_type == 14):
            merged_emiss_bus = gpd.sjoin(
                gdf_stop_sum_emiss_buses[['to_lat', 'to_lon', 'to_stop_name', 'nmvoc', 'geometry']], cell, how='left',
                op='within')
            merged_emiss_bus.rename({'nmvoc': 'emiss'}, axis=1, inplace=True)

        elif (selected_emission_type == 15):
            merged_emiss_bus = gpd.sjoin(
                gdf_stop_sum_emiss_buses[['to_lat', 'to_lon', 'to_stop_name', 'pm', 'geometry']], cell, how='left',
                op='within')
            merged_emiss_bus.rename({'pm': 'emiss'}, axis=1, inplace=True)

        elif (selected_emission_type == 16):
            merged_emiss_bus = gpd.sjoin(
                gdf_stop_sum_emiss_buses[['to_lat', 'to_lon', 'to_stop_name', 'pm_nexh', 'geometry']], cell, how='left',
                op='within')
            merged_emiss_bus.rename({'pm_nexh': 'emiss'}, axis=1, inplace=True)

        elif (selected_emission_type == 17):
            merged_emiss_bus = gpd.sjoin(
                gdf_stop_sum_emiss_buses[['to_lat', 'to_lon', 'to_stop_name', 'co2eq', 'geometry']], cell, how='left',
                op='within')
            merged_emiss_bus.rename({'co2eq': 'emiss'}, axis=1, inplace=True)

        elif (selected_emission_type == 18):
            merged_emiss_bus = gpd.sjoin(
                gdf_stop_sum_emiss_buses[['to_lat', 'to_lon', 'to_stop_name', 'co2eq_wtt', 'geometry']], cell, how='left',
                op='within')
            merged_emiss_bus.rename({'co2eq_wtt': 'emiss'}, axis=1, inplace=True)



        # merged_emiss_bus['sum_emiss'] = 1
        # Compute stats per grid cell -- aggregate number of vehicles to grid cells with dissolve
        dissolve_emiss = merged_emiss_bus.dissolve(by="index_right", aggfunc="sum")
        cell.loc[dissolve_emiss.index, 'emiss'] = dissolve_emiss.emiss.values
        ## ---->> save GRIDDED data ------------#####
        centroids = np.column_stack((cell.centroid.x, cell.centroid.y))
        centroids = pd.DataFrame(centroids, columns=["x", "y"])
        gridded_data = pd.DataFrame({'x': centroids.x,
        ...
        ## save file
        with open(path_app + "static/gridded_emissions.txt", "w") as file:
            file.write(str(joined_lista))
        with open(path_app + "static/gridded_emissions.txt", "r+") as f:
            old = f.read()  # read everything in the file
            f.seek(0)  # rewind
            f.write("var emission = \n" + old)  # assign the "var name" in the .geojson file

        min_emiss_bus = round(min(gridded_data.z), ndigits=1)
        max_emiss_bus = round(max(gridded_data.z), ndigits=1)

        session["min_emiss_bus"] = min_emiss_bus
        session["max_emiss_bus"] = max_emiss_bus
        print(min_emiss_bus)
        print(max_emiss_bus)


        with open(path_app + "static/min_emiss_bus.txt", "w") as file:
            file.write(str(min_emiss_bus))
        with open(path_app + "static/min_emiss_bus.txt", "r+") as f:
            old = f.read()  # read everything in the file
            f.seek(0)  # rewind
            f.write("var min_emiss_bus = \n" + old)  # assign the "var name" in the .geojson file

        with open(path_app + "static/max_emiss_bus.txt", "w") as file:
            file.write(str(max_emiss_bus))
        with open(path_app + "static/max_emiss_bus.txt", "r+") as f:
            old = f.read()  # read everything in the file
            f.seek(0)  # rewind
            f.write("var max_emiss_bus = \n" + old)  # assign the "var name" in the .geojson file
        '''
        ################################################################################
        ################################################################################
        ################################################################################
        ...
        # GTFS_LEGS = emiss_gtfs.groupby(['geom', 'from_stop_name', 'to_stop_name', 'km']).size().reset_index().rename(columns={0: 'counts'})
        # GTFS_LEGS['index_LEGS'] = GTFS_LEGS.index
        # GTFS_LEGS = GTFS_LEGS[GTFS_LEGS.km != 0]
        ...
        """
        gtfs_emiss_LEGS_all['ec'] = round(
            gtfs_emiss_LEGS_all['ec'], 2)
        gtfs_emiss_LEGS_all['ec_wtt'] = round(
            gtfs_emiss_LEGS_all['ec_wtt'], 2)
        gtfs_emiss_LEGS_all['nox'] = round(
            gtfs_emiss_LEGS_all['nox'], 2)
        gtfs_emiss_LEGS_all['nmvoc'] = round(
            gtfs_emiss_LEGS_all['nmvoc'], 2)
        gtfs_emiss_LEGS_all['pm'] = round(
            gtfs_emiss_LEGS_all['pm'], 4)
        gtfs_emiss_LEGS_all['pm_nexh'] = round(
            gtfs_emiss_LEGS_all['pm_nexh'], 4)
        gtfs_emiss_LEGS_all['co2_eq'] = round(
            gtfs_emiss_LEGS_all['co2_eq'], 2)
        gtfs_emiss_LEGS_all['co2_eq_wtt'] = round(
            gtfs_emiss_LEGS_all['co2_eq_wtt'], 2)
        gtfs_emiss_LEGS_all['temperature'] = round(
            gtfs_emiss_LEGS_all['temperature'], 2)
        gtfs_emiss_LEGS_all['AC_cons'] = round(
            gtfs_emiss_LEGS_all['AC_cons'], 4)
        gtfs_emiss_LEGS_all['heating_cons'] = round(
            gtfs_emiss_LEGS_all['heating_cons'], 4)
        """
...
def FCD_heatmap_hourrange_selector():
    ...
    if request.method == "POST":
        ...
        ### ----- DESTINATIONS ---------------------------------- ##################################################
        ## transform the "destinations" table into a geodataframe
        # geometry = [Point(xy) for xy in zip(routes_destination.lon, routes_destination.lat)]
        ...
        merged_full = gpd.sjoin(gdf, cell, how='left', predicate='within')
        ## remove geom
        ...
        # make a simple count variable that we can sum
        merged['n_vehicles'] = 1
        merged[["id_veh"]] =merged[["id_veh"]].apply(pd.to_numeric)
        # Compute stats per grid cell -- aggregate number of vehicles to grid cells with dissolve
        ...
        # AAA = pd.DataFrame(dissolve_means)
        # put this into cells
...
def timeline_heatmaps_hourly():
    if request.method == "POST":
        ...
        return render_template("timeline_heatmaps_FCD.html", data_timeline = data_timeline)
"""
    ...
    if request.method == "POST":
        ...
        for i in range(24):
            ...
            gridded_data_triptime = pd.DataFrame(
                {'y': gridded_data_triptime['latitude'], 'x': gridded_data_triptime['longitude'], 'z': gridded_data_triptime['triptime_s']})
            ...
            gridded_data_tripdistance = pd.DataFrame(geo_destination_routes[['latitude', 'longitude', 'tripdistance_m']])
        ...
        with open(path_app + "templates/hourly_heatmap_ROMA_N_vehicles.html", 'r') as file:
            ...
            data = file.readlines()
            data[34] = str(
                'header, nav { left: 2%; top: 85%; z-index: 700; position: absolute; text-align: center; margin: 20px 0; padding: 10px; border-spacing: 5px;}') + \
                    str('nav a {text-decoration: none; background: #000000;  border-radius: 5px;font-size: 1.2em; color: white;padding: 3px 8px;  display: table-cell;}') + \
                    ...
                    str(" main {background-color: rgba(51, 170, 51, .3); left: 2%;	top: 10%; z-index: 700;	position: absolute;	margin: 2px 0;padding: 5px;	text-overflow:ellipsis; overflow:hidden; width: 280px;  height: 300px}") + \
        ...
        with open(path_app + "templates/hourly_heatmap_ROMA_N_vehicles.html", 'r') as file:
            # read a list of lines into data
            data = file.readlines()
            data[35] = str('<main class="inline-block-center"> \n') + \
                       ...
                       str('<a class="col_privata" href="/mob_privata_page/" style="color:#ffffff;">Private Mobility</a> \n') + \
                       str('<a class="col_collettivo" href="/public_transport_page/">Public Transport</a> \n') + \
                       str('<a class="col_scenari" href="/TRIP_LEGS/">Avoid-Shift measures</a> \n') + \
                       str('<a href="/charging_infrastructure_OSM/">EVs Charging Profile</a> \n') + \
                       str('<a class="col_impatti" href="/public_transport_page/">Impacts</a> \n') + \
                       str('</nav> \n') + \
                       str('<script src="https://cdn.jsdelivr.net/npm/iso8601-js-period@0.2.1/iso8601.min.js"></script> \n')
        with open(path_app + 'templates/hourly_heatmap_ROMA_N_vehicles.html', 'w') as file:
            file.writelines(data)
...
def charging_infrastructure_OSM():
    ...
    stored_scenario_files = glob.glob(path_app + "static/params/selected_scenario_*.txt")
    stored_scenario_files.sort(key=os.path.getmtime)
    stored_scenario_file = stored_scenario_files[len(stored_scenario_files) - 1]
    with open(stored_scenario_file) as file:
        selected_scenario = file.read()
    print("selected_scenario-------I AM HERE-----------: ", selected_scenario)
    session["scenario_type"] = selected_scenario
    print(session["scenario_type"])
...
def dow_selector():
    ...
    # selected_dow = 17
    # selected_scenario = 21

    try:
        with open(path_app + "static/params/selected_DOW_" + session.sid + ".txt", "r") as file:
            selected_dow = file.read()
        print("selected_dow-------I AM HERE-----------: ", selected_dow)
    except:
        stored_selected_dow_files = glob.glob(path_app + "static/params/selected_DOW_*.txt")
        stored_selected_dow_files.sort(key=os.path.getmtime)
        stored_selected_dow_file = stored_selected_dow_files[len(stored_selected_dow_files) - 1]
        with open(stored_selected_dow_file) as file:
            selected_dow = file.read()
        print("selected_dow-------I AM HERE-----------: ", selected_dow)
...
@app.route('/scenario_selector/', methods=['GET', 'POST'])
def scenario_selector():
    ...
    try:
        with open(path_app + "static/params/selected_scenario_" + session.sid + ".txt", "r") as file:
            selected_scenario = file.read()
        print("selected_scenario-------I AM HERE-----------: ", selected_scenario)
    except:
        stored_scenario_files = glob.glob(path_app + "static/params/selected_scenario_*.txt")
    ...
    if request.method == "POST":
        session["scenario_type"] = request.form.get("scenario_type")
        selected_scenario = session["scenario_type"]
        print("selected_scenario:", selected_scenario)
        selected_scenario = str(selected_scenario)
        print("i am here")

        selected_scenario = int(selected_scenario)
        if selected_scenario == 21:
            scenario = "scen1"
            print("scen1")
        elif selected_scenario == 22:
            scenario = "scen2"
            print("scen2")

        print("scenario-----:", selected_scenario)

        with open(path_app + "static/params/selected_scenario_" + session.sid + ".txt", "w") as file:
            file.write(str(selected_scenario))

        ### -----> make QUERY of RECHARGE DATA --- or read input data ----##############################
        #### ------------------------ ###############
        # df_steps_recharge = pd.read_csv(path_app + 'static/recharge_steps.csv', usecols=[1, 2, 3, 4, 5,6])
        ...
        query_recharge_profiles_fcd = text(''' SELECT * FROM test.recharge_profiles_fcd
                                                     WHERE dow = :yy AND
                                                     scenario_type =:xx ''')

        stmt = query_recharge_profiles_fcd.bindparams(yy=str(dow), xx=str(scenario))

        print("----- I AM AFTER the QUERY---- DOW and SCENARIO")
        ...
        aggr_steps_recharge = df_steps_recharge[['zmu', 'mean_energy', 'index_zmu']].groupby(['zmu', 'index_zmu'],
                                                 sort=False).sum().reset_index()
        aggr_steps_recharge['mean_energy'] = round(aggr_steps_recharge.mean_energy, ndigits=4)

        ### merge with staypoints by zones
        df_steps_recharge_zones = df_steps_recharge.drop_duplicates(subset=["zmu"])
        aggr_steps_recharge = pd.merge(aggr_steps_recharge, df_steps_recharge_zones[['zmu', 'homes']], on=['zmu'], how='left')
        aggr_steps_recharge['tot_staypoints'] = round(aggr_steps_recharge.homes, ndigits=0)
        ...
        aggr_steps_recharge = aggr_steps_recharge[['zmu','mean_energy', 'index_zmu', 'homes',
        ...
        ##### --->>>>>> make the sessions data...<<<------------############################################
        ### convert Geodataframe into .geojson...
        session["home_staypoints"] = gdf_staypoints.to_json()
...
def figure_plotly30():
    ...
    # df_steps_recharge = pd.read_csv(path_app + 'static/recharge_steps_de8b9a6f-ce79-4600-a529-4aa45ed9eab4.csv')
    ...
    filtered_ZMU['power_type'] = filtered_ZMU['power_type'].replace(['Ot', 'Hs', 'Hm'], ['Other', 'House', 'Home'])


    fig = px.bar(filtered_ZMU, x='time_steps', y="mean_power", color="power_type", labels={"mean_power": "power(kW)"} ,
                 color_discrete_map={
                     'Other': 'yellow',
                     'Home': 'blue',
                     'House': 'red'
    ...
    fig.update_yaxes(range=[0, 2*max(filtered_ZMU.mean_power)])
    fig.update_layout(
      plot_bgcolor='white')
    ##----> delete files within the folder "/templates/plotly_plots. ---######
    import os
...
def OD_path_vehicle():
    ...
    """
    ## clean all files into the directory /'static/ORIGIN_DESTINATIONS'
    import os
    import glob
    files_to_remove = glob.glob(path_app + 'static/ORIGIN_DESTINATIONS/*')
    for f in files_to_remove:
        os.remove(f)
    """
...
def insert_ORIGIN_name():
    ...
    return render_template("index_OD_path_vehicles_ORIGIN_point.html", data_location_ORIGIN=data_location_ORIGIN, session_ORIGIN_point=session["ORIGIN_point"])
    ...
    try:
        ...
        try:
            ...
            if lr != 0:
                ...
                ####################################################################################
                # save first as geojson file
                edges_shortest_route_cost_gdf.geometry.to_file(filename=path_app + 'static/OD_path_vehicle.geojson', driver='GeoJSON')
...
def best():
  ...
  gtfslist1 = [allgtfs['file'].values[0]] + glob.glob(path_app + 'static/best/gtfs/*.zip')
  file = open(path_app + 'static/best/gtfslist1.txt', 'w'); file.write('var gtfs_zip1 = ' + str(gtfslist1)); file.close()
  gtfslist2 = [allgtfs['file'].values[1]] + glob.glob('./static/best/gtfs/*.zip')
  file = open(path_app + 'static/best/gtfslist2.txt', 'w'); file.write('var gtfs_zip2 = ' + str(gtfslist2)); file.close()
  gtfslist3 = [allgtfs['file'].values[2]] + glob.glob('./static/best/gtfs/*.zip')
  file = open(path_app + 'static/best/gtfslist3.txt', 'w'); file.write('var gtfs_zip3 = ' + str(gtfslist3)); file.close()
  gtfslist4 = [allgtfs['file'].values[3]] + glob.glob('./static/best/gtfs/*.zip')
  file = open(path_app + 'static/best/gtfslist4.txt', 'w'); file.write('var gtfs_zip4 = ' + str(gtfslist4)); file.close()
  gtfslist5 = [allgtfs['file'].values[4]] + glob.glob('./static/best/gtfs/*.zip')
  file = open(path_app + 'static/best/gtfslist5.txt', 'w'); file.write('var gtfs_zip5 = ' + str(gtfslist5)); file.close()
  gtfslist6 = [allgtfs['file'].values[5]] + glob.glob('./static/best/gtfs/*.zip')
  file = open(path_app + 'static/best/gtfslist6.txt', 'w'); file.write('var gtfs_zip6 = ' + str(gtfslist6)); file.close()

  file = open(path_app +'static/best/dswlist.txt', 'w');file.write('var seldate1 = ["'+str(allgtfs['data'].values[0])+'"]');file.close()
  file = open(path_app + 'static/best/dsplist.txt', 'w');file.write('var seldate2 = ["'+str(allgtfs['data'].values[1])+'"]');file.close()
  file = open(path_app + 'static/best/dshlist.txt', 'w');file.write('var seldate3 = ["'+str(allgtfs['data'].values[2])+'"]');file.close()
  file = open(path_app + 'static/best/dwwlist.txt', 'w');file.write('var seldate4 = ["'+str(allgtfs['data'].values[3])+'"]');file.close()
  file = open(path_app + 'static/best/dwplist.txt', 'w');file.write('var seldate5 = ["'+str(allgtfs['data'].values[4])+'"]');file.close()
  file = open(path_app +'static/best/dwhlist.txt', 'w');file.write('var seldate6 = ["'+str(allgtfs['data'].values[5])+'"]');file.close()

  if os.path.isfile(path_app +'static/best/city.csv'):
      with open(path_app +'static/best/city.csv', "r") as text_file:
          old_city = text_file.read()
  else:
      old_city = 'Select city'
  with open(path_app +'static/best/citylist.txt', "w") as text_file:
  ...
  if not os.path.isfile(path_app + 'static/best/best_stops.geojson'):
      file = open(path_app + 'static/best/best_stops.geojson', 'w')
...
def GTFS_chooser(which):
    ...
    if request.method == "POST":
        ...
        # List all gtfs files and write them on txt file
        gtfslist1 = [allgtfs['file'].values[0]] + glob.glob(path_app + 'static/best/gtfs/*.zip')
        file = open(path_app + 'static/best/gtfslist1.txt', 'w');
        file.write('var gtfs_zip1 = ' + str(gtfslist1));
        file.close()
        gtfslist2 = [allgtfs['file'].values[1]] + glob.glob(path_app + 'static/best/gtfs/*.zip')
        file = open(path_app + 'static/best/gtfslist2.txt', 'w');
        file.write('var gtfs_zip2 = ' + str(gtfslist2));
        file.close()
        gtfslist3 = [allgtfs['file'].values[2]] + glob.glob(path_app + 'static/best/gtfs/*.zip')
        file = open(path_app + 'static/best/gtfslist3.txt', 'w');
        file.write('var gtfs_zip3 = ' + str(gtfslist3));
        file.close()
        gtfslist4 = [allgtfs['file'].values[3]] + glob.glob(path_app + 'static/best/gtfs/*.zip')
        file = open(path_app + 'static/best/gtfslist4.txt', 'w');
        file.write('var gtfs_zip4 = ' + str(gtfslist4));
        file.close()
        gtfslist5 = [allgtfs['file'].values[4]] + glob.glob(path_app + 'static/best/gtfs/*.zip')
        file = open(path_app + 'static/best/gtfslist5.txt', 'w');
        file.write('var gtfs_zip5 = ' + str(gtfslist5));
        file.close()
        gtfslist6 = [allgtfs['file'].values[5]] + glob.glob(path_app + 'static/best/gtfs/*.zip')
        file = open(path_app + 'static/best/gtfslist6.txt', 'w');
        ...
        if which == '<sumw>': file = open(path_app + 'static/best/dswlist.txt', 'w'); file.write('var seldate1 = ' + str(sibydl)); file.close()
        if which == '<sump>': file = open(path_app + 'static/best/dsplist.txt', 'w'); file.write('var seldate2 = ' + str(sibydl)); file.close()
        if which == '<sumh>': file = open(path_app + 'static/best/dshlist.txt', 'w'); file.write('var seldate3 = ' + str(sibydl)); file.close()
        if which == '<winw>': file = open(path_app + 'static/best/dwwlist.txt', 'w'); file.write('var seldate4 = ' + str(sibydl)); file.close()
        if which == '<winp>': file = open(path_app + 'static/best/dwplist.txt', 'w'); file.write('var seldate5 = ' + str(sibydl)); file.close()
        if which == '<winh>': file = open(path_app + 'static/best/dwhlist.txt', 'w'); file.write('var seldate6 = ' + str(sibydl)); file.close()
...
def date_chooser(which):
    ...
    if request.method == "POST":
        ...
        allgtfs.to_csv(path_app + 'static/best/gtfsdata.csv', index=False)
        if which == '<datesw>': file = open(path_app + 'static/best/dswlist.txt', 'r'); sibydl = ast.literal_eval(file.read().split('= ')[-1]); sibydl[0] = selected_date_data; file = open(path_app + 'static/best/dswlist.txt', 'w'); file.write('var seldate1 = ' + str(sibydl)); file.close()
        if which == '<datesp>': file = open(path_app + 'static/best/dsplist.txt', 'r'); sibydl = ast.literal_eval(file.read().split('= ')[-1]); sibydl[0] = selected_date_data; file = open(path_app + 'static/best/dsplist.txt', 'w'); file.write('var seldate2 = ' + str(sibydl)); file.close()
        if which == '<datesh>': file = open(path_app + 'static/best/dshlist.txt', 'r'); sibydl = ast.literal_eval(file.read().split('= ')[-1]); sibydl[0] = selected_date_data; file = open(path_app + 'static/best/dshlist.txt', 'w'); file.write('var seldate3 = ' + str(sibydl)); file.close()
        if which == '<dateww>': file = open(path_app + 'static/best/dwwlist.txt', 'r'); sibydl = ast.literal_eval(file.read().split('= ')[-1]); sibydl[0] = selected_date_data; file = open(path_app + 'static/best/dwwlist.txt', 'w'); file.write('var seldate4 = ' + str(sibydl)); file.close()
        if which == '<datewp>': file = open(path_app + 'static/best/dwplist.txt', 'r'); sibydl = ast.literal_eval(file.read().split('= ')[-1]); sibydl[0] = selected_date_data; file = open(path_app + 'static/best/dwplist.txt', 'w'); file.write('var seldate5 = ' + str(sibydl)); file.close()
        if which == '<datewh>': file = open(path_app + 'static/best/dwhlist.txt', 'r'); sibydl = ast.literal_eval(file.read().split('= ')[-1]); sibydl[0] = selected_date_data; file = open(path_app + 'static/best/dwhlist.txt', 'w'); file.write('var seldate6 = ' + str(sibydl)); file.close()
...
def city_chooser():
    if request.method == "POST":
        ...
        with open(path_app + 'static/best/city.csv', "w") as text_file:
...
def best_prep():
    ...
    import sys
    sys.path.insert(0, path_app + 'static/best/swt')
    ...
    if request.method == "POST":
        ...
        if which == '<input>':
            ...
            if name == '':
                ...
                fileout = Path(path_app , 'static/best/swt/Out/input_new.xlsx')
        ...
        if which == '<gtfs>':
            ...
            if name == '':
                ...
                fileout0 = Path(path_app, 'static/best/swt/Out/mytrips_' + mycity + '_' + str(allgtfs['data'].values[0])[0:4] + '-' + str(allgtfs['data'].values[0])[4:6] + '-' + str(allgtfs['data'].values[0])[6:8] + '.csv')
                fileout1 = Path(path_app, 'static/best/swt/Out/mytrips_' + mycity + '_' + str(allgtfs['data'].values[1])[0:4] + '-' + str(allgtfs['data'].values[1])[4:6] + '-' + str(allgtfs['data'].values[1])[6:8] + '.csv')
                fileout2 = Path(path_app, 'static/best/swt/Out/mytrips_' + mycity + '_' + str(allgtfs['data'].values[2])[0:4] + '-' + str(allgtfs['data'].values[2])[4:6] + '-' + str(allgtfs['data'].values[2])[6:8] + '.csv')
                fileout3 = Path(path_app, 'static/best/swt/Out/mytrips_' + mycity + '_' + str(allgtfs['data'].values[3])[0:4] + '-' + str(allgtfs['data'].values[3])[4:6] + '-' + str(allgtfs['data'].values[3])[6:8] + '.csv')
                fileout4 = Path(path_app, 'static/best/swt/Out/mytrips_' + mycity + '_' + str(allgtfs['data'].values[4])[0:4] + '-' + str(allgtfs['data'].values[4])[4:6] + '-' + str(allgtfs['data'].values[4])[6:8] + '.csv')
                fileout5 = Path(path_app, 'static/best/swt/Out/mytrips_' + mycity + '_' + str(allgtfs['data'].values[5])[0:4] + '-' + str(allgtfs['data'].values[5])[4:6] + '-' + str(allgtfs['data'].values[5])[6:8] + '.csv')
...
def caricadadb(which):
    ...
    if request.method == "POST":
        ...
        if which == '<input>':
            ...
            writer = pd.ExcelWriter(Path(path_app, 'static/best/swt/Out/input_new.xlsx'), engine='xlsxwriter')
        ...
        if which == '<output>':
            ...
            writer = pd.ExcelWriter(Path(path_app, 'static/best/swt/Out/output_economici.xlsx'), engine='xlsxwriter')
            fileout1.to_excel(writer, sheet_name='output_economici',index=False)
            writer.close()
            writer = pd.ExcelWriter(Path(path_app, 'static/best/swt/Out/output_tecnici.xlsx'), engine='xlsxwriter')
            fileout2.to_excel(writer, sheet_name='output_tecnici',index=False)
            writer.close()
            writer = pd.ExcelWriter(Path(path_app, 'static/best/swt/Out/cp_dati_anno_linea.xlsx'), engine='xlsxwriter')
            fileout3.to_excel(writer, sheet_name='cp_dati_anno_linea',index=False)
            writer.close()
            writer = pd.ExcelWriter(Path(path_app, 'static/best/swt/Out/dati_orari_rete.xlsx'), engine='xlsxwriter')
            fileout4.to_excel(writer, sheet_name='dati_orari_rete',index=False)
            writer.close()
            writer = pd.ExcelWriter(Path(path_app, 'static/best/swt/Out/cp_dati_orari_cap.xlsx'), engine='xlsxwriter')
...
def launch_best():
    print('best!!')
    DB_input = pd.read_excel(path_app + 'static/best/swt/out/input_new.xlsx', sheet_name=None)
    import sys
    sys.path.insert(0, path_app + 'static/best/swt')
    ...
    DB_output = best.run()
    DB_output["dati_orari_rete"].to_excel(path_app + 'static/best/swt/out/dati_orari_rete.xlsx', sheet_name='dati_orari_rete', index=False)
    DB_output["cp_dati_anno_linea"].to_excel(path_app + 'static/best/swt/out/cp_dati_anno_linea.xlsx', sheet_name='cp_dati_anno_linea',
                                             index=False)
    DB_output["output_tecnici"].to_excel(path_app + 'static/best/swt/out/output_tecnici.xlsx', sheet_name='output_tecnici', index=False)
    DB_output["output_economici"].to_excel(path_app + 'static/best/swt/out/output_economici.xlsx', sheet_name='output_economici',
                                           index=False)
    DB_output["cp_dati_orari_cap"].to_excel(path_app + 'static/best/swt/out/cp_dati_orari_cap.xlsx', sheet_name='cp_dati_orari_cap',
                                            index=False)
...
if __name__ == '__main__':
    ...
    app.run(host="192.168.134.219", port=3000, debug=True,  threaded=True, use_reloader=False)
